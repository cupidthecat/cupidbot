...
public class Rs2ReflectionGroundItemActionsIntegrationTest {
    ...
    public static void startClientAndLogin() throws Exception {
        ...
        log.info("Waiting for CupidBot to initialize...");
        waitForCondition("CupidBot.getClientThread() != null", 90, () ->
                CupidBot.getClientThread() != null
        );
        log.info("CupidBot initialized.");
    ...
    private static void performLogin() throws Exception {
        for (int attempt = 1; attempt <= MAX_LOGIN_ATTEMPTS; attempt++) {
            ...
            if (waitForConditionSafe("Login", 20, CupidBot::isLoggedIn)) {
    ...
    public void testGetGroundItemActionsContainsTake() throws Exception {
        ...
        assertTrue("Must be logged in", CupidBot.isLoggedIn());

        boolean foundTake = false;

        for (int itemId : TEST_ITEM_IDS) {
            final int id = itemId;
            ItemComposition itemComp = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                    CupidBot.getClient().getItemDefinition(id)
            ).orElse(null);
    ...
    public void testRetryDiscoveryAfterCacheInvalidation() throws Exception {
        ...
        ItemComposition firstItem = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                CupidBot.getClient().getItemDefinition(TEST_ITEM_IDS[0])
        ).orElse(null);
