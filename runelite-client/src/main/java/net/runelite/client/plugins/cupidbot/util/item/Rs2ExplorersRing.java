package net.runelite.client.plugins.cupidbot.util.item;
...
public class Rs2ExplorersRing {
    ...
    private static int getTotalChargesExplorersRing(String key) {
        Integer charges = CupidBot.getConfigManager().getConfiguration(ItemChargeConfig.GROUP, key, Integer.class);
        if (charges != null) {
            // Migrate configuration to RSProfile
            CupidBot.getConfigManager().unsetConfiguration(ItemChargeConfig.GROUP, key);
            CupidBot.getConfigManager().setRSProfileConfiguration(ItemChargeConfig.GROUP, key, charges);
            return charges;
        }

        // Retrieve charges from RSProfile configuration
        charges = CupidBot.getConfigManager().getRSProfileConfiguration(ItemChargeConfig.GROUP, key, Integer.class);
    ...
    public static boolean closeInterface() {
        if (CupidBot.getVarbitValue(VarbitID.LUMBRIDGE_ALCHEMY_HIGH) == 0) return true;
