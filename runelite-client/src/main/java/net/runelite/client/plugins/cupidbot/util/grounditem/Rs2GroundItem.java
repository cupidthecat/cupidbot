...
public class Rs2GroundItem {
    ...
    public static boolean runWhilePaused(BooleanSupplier booleanSupplier) {
        final boolean paused = CupidBot.pauseAllScripts.getAndSet(true);
        final boolean success = booleanSupplier.getAsBoolean();
        if (!paused && !CupidBot.pauseAllScripts.compareAndSet(true, false)) {
    ...
    private static boolean interact(RS2Item rs2Item, String action) {
        ...
        try {
            ...
            CupidBot.logStackTrace("Rs2GroundItem", ex);
        }
        return true;
    }
    ...
    private static boolean interact(InteractModel groundItem, String action) {
        ...
        try {
            ...
            item = CupidBot.getClientThread().runOnClientThreadOptional(() -> CupidBot.getClient().getItemDefinition(groundItem.getId())).orElse(null);
            if (item == null) return false;
            identifier = groundItem.getId();

            LocalPoint localPoint = LocalPoint.fromWorld(CupidBot.getClient(), groundItem.getLocation());
    ...
    public static RS2Item[] getAllAt(int x, int y) {
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            if (!CupidBot.isLoggedIn()) return EMPTY_ARRAY;
            ...
            return groundItems.stream()
                    .map(groundItem -> new RS2Item(CupidBot.getItemManager().getItemComposition(groundItem.getId()), tile, groundItem))
                    .toArray(RS2Item[]::new);
        }).orElse(EMPTY_ARRAY);
    }
    ...
    public static RS2Item[] getAllFromWorldPoint(int range, WorldPoint worldPoint) {
        ...
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
                    ...
                    return temp.stream().sorted(Comparator.comparingInt(value -> value.getTile().getLocalLocation()
                                    .distanceTo(CupidBot.getClient().getLocalPlayer().getLocalLocation())))
                            .toArray(RS2Item[]::new);
                }).orElse(EMPTY_ARRAY);
    }
    ...
    public static boolean lootItemBasedOnValue(int value, int range) {
        ...
        final long[] prices = CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            long[] result = new long[items.length];
            for (int i = 0; i < items.length; i++) {
                result[i] = (long) CupidBot.getItemManager().getItemPrice(items[i].getItem().getId()) * items[i].getTileItem().getQuantity();
            }
            return result;
    ...
    public static boolean lootAllItemBasedOnValue(int value, int range) {
        RS2Item[] groundItems = CupidBot.getClientThread().runOnClientThreadOptional(() ->
        ...
        for (RS2Item rs2Item : groundItems) {
            ...
            long totalPrice = (long) CupidBot.getClientThread().runOnClientThreadOptional(() ->
                    CupidBot.getItemManager().getItemPrice(rs2Item.getItem().getId()) * rs2Item.getTileItem().getQuantity()).orElse(0);
            if (totalPrice >= value) {
    ...
    public static boolean hasLineOfSight(Tile tile) {
        ...
        return tile.getWorldLocation().toWorldArea()
                .hasLineOfSightTo(CupidBot.getClient().getTopLevelWorldView(), Rs2Player.getWorldLocation().toWorldArea());
    }
