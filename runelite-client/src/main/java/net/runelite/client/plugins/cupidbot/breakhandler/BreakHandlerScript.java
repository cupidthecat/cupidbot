package net.runelite.client.plugins.cupidbot.breakhandler;
...
import net.runelite.client.plugins.cupidbot.util.events.PluginPauseEvent;
...
public class BreakHandlerScript extends Script {
    ...
    private void handleInitiatingBreakState() {
        ...
        preBreakWorld = CupidBot.getClientThread().runOnClientThreadOptional(
                () -> CupidBot.getClient().getWorld()).orElse(0);
    ...
    private void resetConfigToggles() {
        ...
        CupidBot.getConfigManager().setConfiguration(BreakHandlerConfig.configGroup, "breakNow", false);
        CupidBot.getConfigManager().setConfiguration(BreakHandlerConfig.configGroup, "breakEndNow", false);
    }
    ...
    private void updateBreakTimers() {
        ...
        if (state == BreakHandlerState.WAITING_FOR_BREAK && breakIn >= 0 && CupidBot.isLoggedIn()) {
    ...
    private void checkForBan() {
        Optional<Integer> loginIdx = CupidBot.getClientThread().runOnClientThreadOptional(
                () -> CupidBot.getClient().getGameState() == GameState.LOGIN_SCREEN
                        ? CupidBot.getClient().getLoginIndex()
                        : -1);
    ...
    private void updatePlayerNameCache() {
        boolean currentlyLoggedIn = CupidBot.isLoggedIn();
    ...
    private void shutdownPlugin() {
        try {
            ...
            CupidBot.stopPlugin(BreakHandlerPlugin.class);
