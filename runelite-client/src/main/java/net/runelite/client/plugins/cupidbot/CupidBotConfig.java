...
@ConfigGroup(CupidBotConfig.configGroup)
public interface CupidBotConfig extends Config
{
	String configGroup = "cupidbot";

	@ConfigSection(
		name = "General",
		description = "The overall global settings for cupidbot",
		position = 0
	)
	...
	String keyOnlyCupidBotLogging = "onlyCupidBotLogging";
	@ConfigItem(
		keyName = keyOnlyCupidBotLogging,
		name = "Only CupidBot Logs",
		description = "Show only CupidBot plugin logs in game chat (filters out other RuneLite logs)",
		position = 3,
		section = loggingSection
	)
	default boolean onlyCupidBotLogging() {
		return false;
	}
	...
	@ConfigItem(
		...
		description = "Reserved for compatibility. CupidBot project telemetry is always disabled.",
		position = 6,
		section = generalSection
	)
