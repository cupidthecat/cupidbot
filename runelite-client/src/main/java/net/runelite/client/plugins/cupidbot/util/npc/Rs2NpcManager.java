...
public class Rs2NpcManager {
    ...
    private static <T> Map<Integer, T> loadJsonFile(String filename, Type typeToken) {
        ...
        try (InputStream inputStream = Rs2NpcStats.class.getResourceAsStream(filename)) {
            if (inputStream == null) {
                CupidBot.log(Level.ERROR,"Failed to load " + filename);
    ...
    private static void loadNpcLocationsByName()
    {
        ...
        if (rawMap == null)
        {
            CupidBot.log(Level.INFO,"No location data found in {}", filename);
        ...
        CupidBot.log("Loaded {} NPC names with location data from {}", locationMap.size(), filename);
    }
    ...
    public static MonsterLocation getClosestLocation(String npcName, int minClustering, boolean avoidWilderness) {
        ...
        CupidBot.log(Level.INFO,"Finding closest location for: " + npcName);

        List<MonsterLocation> allLocations = getNpcLocations(npcName);
        if (allLocations.isEmpty()) {
            CupidBot.log(Level.INFO,"No locations found for " + npcName);
            return null;
        }
        ...
        List<String> allNames = allLocations.stream()
                ...
                .collect(Collectors.toList());
        CupidBot.log(Level.INFO,"Found " + allLocations.size() + " locations for " + npcName + ": " + String.join(", ", allNames));
        ...
        if (validLocations.isEmpty()) {
            CupidBot.log(Level.INFO,"No valid locations after filtering for " + npcName);
            return null;
        }
        ...
        List<WorldPoint> centers = validLocations.stream()
                ...
                .collect(Collectors.toList());
        if (centers.isEmpty()) {
            CupidBot.log(Level.INFO,"Could not compute any centers for " + npcName);
            return null;
        }
        ...
        if (closest.getCoords().isEmpty()) {
            CupidBot.log(Level.INFO,"Closest location had no coords for " + npcName);
            return null;
        }

        CupidBot.log(Level.INFO,"Closest location for " + npcName + ": " + closest.getLocationName());
