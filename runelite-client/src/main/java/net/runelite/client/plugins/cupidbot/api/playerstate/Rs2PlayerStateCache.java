package net.runelite.client.plugins.cupidbot.api.playerstate;
...
public final class Rs2PlayerStateCache {
	...
	private @Varbit int updateVarbitValue(@Varbit int varbitId) {
		int value;
		value = CupidBot.getClientThread().runOnClientThreadOptional(() -> client.getVarbitValue(varbitId)).orElse(0);
	...
	private @Varp int updateVarpValue(@Varp int varpId) {
		...
		value = CupidBot.getClientThread().runOnClientThreadOptional(() -> client.getVarpValue(varpId)).orElse(0);
