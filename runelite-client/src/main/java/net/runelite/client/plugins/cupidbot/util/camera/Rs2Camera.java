...
public class Rs2Camera {
    ...
    public static void setAngle(int targetDegrees, int maxAngle) {
        ...
        if (CupidBot.isPluginEnabled(CameraPlugin.class)) {
            String configGroup = "zoom";
            String configKey = "cameraSpeed";
            defaultCameraSpeed = CupidBot.getInjector().getInstance(ConfigManager.class).getConfiguration(configGroup, configKey, double.class);
        }
        // Set the camera speed to 3 to make the camera move faster
        CupidBot.getClient().setCameraSpeed(3f);
        ...
        CupidBot.getClient().setCameraSpeed((float) defaultCameraSpeed);
    }
    ...
    public static int getPitch() {
        return fromClientAngleUnits(CupidBot.getClient().getCameraPitch());
    }
    ...
    public static boolean isTileOnScreen(TileObject tileObject) {
        ...
        Polygon poly = Perspective.getCanvasTilePoly(CupidBot.getClient(), tileObject.getLocalLocation());
    ...
    public static int getZoom() {
        return CupidBot.getClient().getVarcIntValue(VarClientInt.CAMERA_ZOOM_RESIZABLE_VIEWPORT);
    }

    public static void setZoom(int zoom) {
        CupidBot.getClientThread().invokeLater(() -> {
            CupidBot.getClient().runScript(ScriptID.CAMERA_DO_ZOOM, zoom, zoom);
        });
    }
    // Get camera/compass facing
    public static int getYaw() {
        return fromClientAngleUnits(CupidBot.getClient().getCameraYaw());
    }
    ...
    private static void setCameraTargetOnClientThread(int target, boolean isPitch) {
        ...
        CupidBot.getClientThread().invoke(() -> {
            if (isPitch) {
                CupidBot.getClient().setCameraPitchTarget(clientTarget);
            } else {
                CupidBot.getClient().setCameraYawTarget(clientTarget);
            }
        });
    }
    ...
    private static void smoothTo(int target, boolean isPitch) {
        ...
        if (Math.abs(delta) < 3 || CupidBot.getClientThread().isClientThread()) {
    ...
    public static boolean isTileCenteredOnScreen(LocalPoint tile, double marginPercentage) {
        Polygon poly = Perspective.getCanvasTilePoly(CupidBot.getClient(), tile);
        if (poly == null) return false;
