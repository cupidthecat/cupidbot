# CupidBot Plugin & Script Development
...
Config UI uses the custom `CupidBotConfigPanel` (`plugins/cupidbot/ui`), not RuneLite's default.

## Threading — hard rules
- Game state reads/writes must happen on the client thread: wrap with `CupidBot.getClientThread().runOnClientThreadOptional(...)` or `.invoke(...)`.
...
- Use singletons via `CupidBot.getRs2XxxCache()` (NPCs, objects, ground items, inventory, bank, varbits, players, widgets, …).
...
- Full reference: `runelite-client/.../cupidbot/api/QUERYABLE_API.md`.
...
- Use `CupidBot.status` for user-visible status; keep logging sparse and at appropriate levels.
