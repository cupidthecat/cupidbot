...
import net.runelite.client.plugins.cupidbot.questhelper.requirements.Requirement;
import net.runelite.client.plugins.cupidbot.questhelper.requirements.item.ItemRequirement;
import net.runelite.client.plugins.cupidbot.questhelper.steps.*;
import net.runelite.client.plugins.cupidbot.questhelper.steps.widget.WidgetHighlight;
...
import net.runelite.client.plugins.cupidbot.util.grandexchange.models.WikiPrice;
...
import net.runelite.client.plugins.cupidbot.api.tileobject.Rs2TileObjectQueryable;
import net.runelite.client.plugins.cupidbot.api.tileobject.models.Rs2TileObjectModel;
import net.runelite.client.plugins.cupidbot.api.tileitem.Rs2TileItemQueryable;
...
public class QuestScript extends Script {
    ...
    public boolean run(QuestHelperConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (getQuestHelperPlugin().getSelectedQuest() != null && !CupidBot.getClientThread().runOnClientThreadOptional(() ->
                    ...
                    boolean isInCutscene = CupidBot.getVarbitValue(4606) > 0;
                    if (isInCutscene) {
	...
	private boolean acquireMissingTradableItems(List<ItemRequirement> missing) {
		...
		if (actionable.isEmpty()) {
			if (!Rs2GrandExchange.isOpen()) {
				CupidBot.status = "Quest helper: heading to Grand Exchange for in-progress offers";
			...
			if (!Rs2GrandExchange.hasBoughtOffer()) {
				CupidBot.status = "Waiting for Grand Exchange offers to fill";
		...
		if (!Rs2Bank.isOpen()) {
			CupidBot.status = "Quest helper: walking to bank for missing items";
		...
		for (ItemRequirement ir : fromBank) {
			...
			CupidBot.status = "Withdrawing " + ir.getName() + " x" + qty;
			Rs2Bank.withdrawX(idToWithdraw, qty);
			if (!sleepUntil(() -> hasItemRequirementOnPlayer(ir), 2_000)) {
				CupidBot.log("Quest helper: bank withdrawal for " + ir.getName() + " did not land in time",
						Level.WARN);
			}
		}
		...
		if (!toBuy.isEmpty()) {
			...
			if (requiredCoins > currentInvCoins) {
				...
				CupidBot.status = "Withdrawing " + coinsToWithdraw + " gp for Grand Exchange";
		...
		if (!Rs2GrandExchange.isOpen()) {
			CupidBot.status = "Quest helper: walking to Grand Exchange";
			Rs2GrandExchange.walkToGrandExchange();
			Rs2GrandExchange.openExchange();
			sleepUntil(Rs2GrandExchange::isOpen, 15_000);
			if (!Rs2GrandExchange.isOpen()) {
		...
		for (ItemRequirement ir : toBuy) {
			...
			CupidBot.status = "Buying " + canonicalName + " x" + qty;
		...
		for (int attempt = 1; attempt <= maxBuyAttempts; attempt++) {
			...
			for (ItemRequirement ir : stillPending) {
				...
				CupidBot.status = "Cancelling unfilled " + canonicalName;
				if (!Rs2GrandExchange.abortOffer(canonicalName, false)) {
					CupidBot.log("Quest helper: failed to abort offer for " + canonicalName
				...
				if (qtyNeeded <= 0) {
					CupidBot.log("Quest helper: " + canonicalName
				...
				if (!Rs2GrandExchange.buyItem(canonicalName, newPrice, qtyNeeded)) {
					CupidBot.log("Quest helper: retry buy for " + canonicalName + " failed to place",
							Level.WARN);
				}
				sleep(800, 1500);
			}
	...
	private List<ItemRequirement> itemsStillBuying(List<ItemRequirement> toBuy,
		...
		GrandExchangeOffer[] offers = CupidBot.getClient().getGrandExchangeOffers();
		if (offers == null) {
			return pending;
		}
	...
	private void collectPurchasedItemsViaBank(List<ItemRequirement> items) {
		CupidBot.status = "Collecting purchased quest items to bank";
		...
		if (!Rs2Bank.isOpen()) {
			CupidBot.status = "Opening bank to retrieve purchased quest items";
			Rs2Bank.walkToBankAndUseBank();
			sleepUntil(Rs2Bank::isOpen, 15_000);
			if (!Rs2Bank.isOpen()) {
				CupidBot.log("Quest helper: failed to open bank after Grand Exchange collection", Level.WARN);
				return;
			}
		}
		...
		for (ItemRequirement ir : items) {
			...
			Rs2Bank.withdrawX(idToWithdraw, qty);
			if (!sleepUntil(() -> hasItemRequirementOnPlayer(ir), 2_000)) {
				CupidBot.log("Quest helper: purchased item " + ir.getName() + " did not land in inventory after bank withdrawal",
						Level.WARN);
			}
		}
	...
	private boolean isItemRequirementTradable(ItemRequirement itemRequirement) {
		return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
			for (Integer id : itemRequirement.getAllIds()) {
				...
				ItemComposition def = CupidBot.getClient().getItemDefinition(id);
	...
	private int tradablePrimaryId(ItemRequirement itemRequirement) {
		List<Integer> tradableIds = CupidBot.getClientThread().runOnClientThreadOptional(() -> {
	...
	private void updateEverHeldItemTracking(QuestHelper selectedQuest) {
		...
		if (questId != heldTrackingQuestId) {
			...
			try {
				state = selectedQuest.getQuest().getState(CupidBot.getClient());
		...
		SwingUtilities.invokeLater(() -> {
			try {
				...
				if (choice >= 0 && choice < selectable.size()) {
					...
					CupidBot.getConfigManager().setConfiguration(
							QuestHelperConfig.QUEST_HELPER_GROUP, "TurnOn", true);
					CupidBot.status = "Vale Totems: using " + selected + " (this session)";
					CupidBot.log("Quest helper: Vale Totems wood type set to " + selected
	...
	private String canonicalItemName(int itemId) {
		return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
			ItemComposition def = CupidBot.getClient().getItemDefinition(itemId);
	...
	private int notedVariantId(int unnotedId) {
		return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
			ItemComposition def = CupidBot.getClient().getItemDefinition(unnotedId);
			if (def == null) {
				return -1;
			}
			if (def.getNote() == 799) {
	...
	private boolean unnoteItemsViaBank(List<ItemRequirement> items) {
		if (!Rs2Bank.isOpen()) {
			CupidBot.status = "Quest helper: walking to bank to un-note items";
			Rs2Bank.walkToBankAndUseBank();
			sleepUntil(Rs2Bank::isOpen, 15_000);
			if (!Rs2Bank.isOpen()) {
				return true;
		...
		for (ItemRequirement ir : items) {
			...
			for (Integer unnotedId : ir.getAllIds()) {
				...
				CupidBot.status = "Depositing noted " + ir.getName();
				...
				CupidBot.status = "Withdrawing " + ir.getName() + " x" + toWithdraw;
				Rs2Bank.withdrawX(unnotedId, toWithdraw);
				if (!sleepUntil(() -> hasItemRequirementOnPlayer(ir), 2_000)) {
					CupidBot.log("Quest helper: un-note withdrawal for " + ir.getName() + " did not land in time",
							Level.WARN);
				}
				break;
			}
	...
	private void stopQuesterWithReason(String reason) {
		CupidBot.status = reason;
		CupidBot.log("Quest helper stopped: " + reason, Level.ERROR);
		if (CupidBot.getConfigManager() != null) {
			CupidBot.getConfigManager().setConfiguration(
	...
	private boolean attemptToAcquireRequirementItem(DetailedQuestStep questStep, ItemRequirement itemRequirement) {
		...
		if (worldPoint != null) {
			if ((Rs2Walker.canReach(worldPoint) && worldPoint.distanceTo(Rs2Player.getWorldLocation()) < 2)
					|| worldPoint.toWorldArea().hasLineOfSightTo(CupidBot.getClient().getTopLevelWorldView(), Rs2Player.getWorldLocation().toWorldArea())
					&& Rs2Camera.isTileOnScreen(LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), worldPoint))) {
	...
	private void notifyMissingRequirement(ItemRequirement itemRequirement) {
		...
		CupidBot.status = "Missing: " + itemName;
		CupidBot.log(String.format("Quest helper missing required item: %s x%d", itemName, quantity), Level.WARN);
	}
    ...
    public boolean applyNpcStep(NpcStep step) {
        ...
        if (npc != null && npc.getLocalLocation() != null && Rs2Camera.isTileOnScreen(npc.getLocalLocation())
                && (CupidBot.getClient().isInInstancedRegion() || (Rs2Walker.canReach(npc.getWorldLocation()) && npc.hasLineOfSight()))) {
    ...
    private String chooseCorrectObjectOption(QuestStep step, Rs2TileObjectModel object) {
        ObjectComposition objComp = CupidBot.getClientThread().runOnClientThreadOptional(() ->
    ...
    private String chooseCorrectNPCOption(QuestStep step, Rs2NpcModel npc) {
        var npcComp = CupidBot.getClientThread().runOnClientThreadOptional(() -> CupidBot.getClient().getNpcDefinition(npc.getId()))
                .orElse(null);
	...
	private boolean hasLineOfSightToObject(Rs2TileObjectModel object) {
		...
		return CupidBot.getClient().getTopLevelWorldView() != null
				&& playerArea.hasLineOfSightTo(CupidBot.getClient().getTopLevelWorldView(), objectArea);
	}
	...
	private boolean hasLineOfSightFrom(WorldPoint point, Rs2TileObjectModel object) {
		...
		return CupidBot.getClient().getTopLevelWorldView() != null
				&& fromArea.hasLineOfSightTo(CupidBot.getClient().getTopLevelWorldView(), targetArea);
	}
    ...
    private boolean applyDetailedQuestStep(DetailedQuestStep conditionalStep) {
        ...
        if (conditionalStep.getIconItemID() != -1
                ...
                && !conditionalStep.getDefinedPoint().getWorldPoint().toWorldArea().hasLineOfSightTo(CupidBot.getClient().getTopLevelWorldView(), Rs2Player.getWorldLocation())) {
    ...
    private boolean applyWidgetStep(WidgetStep step) {
        ...
        var widget = CupidBot.getClient().getWidget(widgetDetails.groupID, widgetDetails.childID);
