package net.runelite.client.plugins.cupidbot.util.tabs;
...
public class Rs2Tab {
    ...
    public static void onVarClientIntChanged(VarClientIntChanged event) {
        ...
        int value = CupidBot.getClient().getVarcIntValue(VarClientID.TOPLEVEL_PANEL);
    ...
    public static boolean switchTo(InterfaceTab tab) {
        ...
        if (tab == InterfaceTab.NOTHING_SELECTED && CupidBot.getVarbitValue(VarbitID.RESIZABLE_STONE_ARRANGEMENT) == 0)
            return false;

        int varcIntIndex = tab.getVarcIntIndex();
        if (varcIntIndex != -1) {
            CupidBot.getClientThread().runOnClientThreadOptional(() -> {
                CupidBot.getClient().runScript(TAB_SWITCH_SCRIPT, varcIntIndex);
                return true;
            });
        } else {
	...
	public static Widget getSpellBookTab()
	{
		...
		{
			if (CupidBot.getVarbitValue(VarbitID.RESIZABLE_STONE_ARRANGEMENT) == 1)
			{
