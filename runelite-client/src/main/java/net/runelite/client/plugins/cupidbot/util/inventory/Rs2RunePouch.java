...
public class Rs2RunePouch
{
	...
	public static void fullUpdate()
	{
		...
		{
			int rawRune = CupidBot.getVarbitValue(RUNE_VARBITS[i]);
			int rawQuantity = CupidBot.getVarbitValue(AMOUNT_VARBITS[i]);
	...
	public static void onWidgetLoaded(WidgetLoaded ev)
	{
		...
		for (int loadOutIndex : RUNEPOUCH_LOADOUT_WIDGETS)
		{
			...
			Widget rootLoadoutWidget = CupidBot.getClient().getWidget(BANK_PARENT_ID, loadOutIndex);
	...
	private static boolean coreLoad(Map<Runes, Integer> requiredRunes)
	{
		...
		for (Map.Entry<Integer, List<PouchSlot>> entry : loadoutSlots.entrySet())
		{
			...
			if (loadoutMap.equals(requiredRunes))
			{
				...
				{
					CupidBot.log("Failed to find loadout widget for index: " + widgetIndex, Level.WARNING);
					break;
				}
				Widget loadWidget = Rs2Widget.findWidget("Load", List.of(parentLoadoutWidget.getStaticChildren()));
				if (loadWidget == null)
				{
					CupidBot.log("Failed to find 'Load' child widget in loadout index: " + widgetIndex, Level.WARNING);
					break;
				}
				...
				CupidBot.doInvoke(menuEntry, loadBounds != null && Rs2UiHelper.isRectangleWithinCanvas(loadBounds) ? loadBounds : Rs2UiHelper.getDefaultRectangle());
		...
		for (Map.Entry<Runes, Integer> entry : requiredRunes.entrySet()) {
			...
			{
				CupidBot.log("Failed to withdraw rune: " + rune + " x" + qty, Level.WARNING);
				return false;
			}
