package net.runelite.client.plugins.cupidbot.util.misc;
...
import net.runelite.client.plugins.cupidbot.util.text.Rs2TextSanitizer;
...
public class Rs2UiHelper {
    ...
    public static boolean isRectangleWithinViewport(Rectangle rectangle) {
        int viewportHeight = CupidBot.getClient().getViewportHeight();
        int viewportWidth = CupidBot.getClient().getViewportWidth();
    ...
    public static boolean isRectangleWithinCanvas(Rectangle rectangle) {
        int canvasHeight = CupidBot.getClient().getCanvasHeight();
        int canvasWidth = CupidBot.getClient().getCanvasWidth();
    ...
    public static Point getClickingPoint(Rectangle rectangle, boolean randomize) {
        ...
        if (Rs2AntibanSettings.naturalMouse) {
            java.awt.Point mousePos = CupidBot.getMouse().getMousePosition();
            return Rs2Random.randomPointEx(new Point(mousePos.x, mousePos.y), rectangle, force);
        } else
            return Rs2Random.randomPointEx(CupidBot.getMouse().getLastClick(), rectangle, force);
    }
    ...
    public static Rectangle getActorClickbox(Actor actor) {
        ...
        Shape clickbox = CupidBot.getClientThread().runOnClientThreadOptional(() -> Perspective.getClickbox(CupidBot.getClient(), CupidBot.getClient().getTopLevelWorldView(), actor.getModel(), actor.getCurrentOrientation(), lp.getX(), lp.getY(),
                Perspective.getTileHeight(CupidBot.getClient(), lp, actor.getWorldLocation().getPlane())))
                .orElse(null);
    ...
    public static Rectangle getObjectClickbox(TileObject object) {
        ...
        Shape clickbox = CupidBot.getClientThread().runOnClientThreadOptional(object::getClickbox).orElse(null);
        if (clickbox == null) return getDefaultRectangle();  //return a small rectangle if clickbox is null
    ...
    public static Rectangle getTileClickbox(Tile tile) {
        ...
        Point screenPoint = Perspective.localToCanvas(CupidBot.getClient(), localPoint, CupidBot.getClient().getPlane());
	 ...
	 *
	 * @deprecated Use {@link #stripTags(String)} or {@link net.runelite.client.plugins.cupidbot.util.text.Rs2TextSanitizer#stripTags(String)}
	...
	public static Rectangle getDefaultRectangle() {
		...
		return new Rectangle(randomValue, randomValue, CupidBot.getClient().getCanvasWidth(), CupidBot.getClient().getCanvasHeight());
	}
	...
	public static boolean isClientVersionCompatible(String minClientVersion) {
		...
		String currentVersion = RuneLiteProperties.getCupidBotVersion();
		if (currentVersion == null) {
			log.warn("Unable to determine current CupidBot version");
			return false;
		}
