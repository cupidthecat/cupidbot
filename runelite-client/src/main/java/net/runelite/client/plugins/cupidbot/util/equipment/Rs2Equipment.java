package net.runelite.client.plugins.cupidbot.util.equipment;
...
public class Rs2Equipment {
     ...
     * the client thread (wrap with {@code CupidBot.getClientThread().invoke(...)}).
     */
    @Deprecated
    public static ItemContainer equipment() {
        assert CupidBot.getClient().isClientThread();
        return CupidBot.getClient().getItemContainer(InventoryID.WORN);
    }
    ...
    public static void storeEquipmentItemsInMemory(ItemContainerChanged e) {
        ...
        for (int i = 0; i < Math.min(itemContainer.getItems().length, ALL_SLOTS.length); i++) {
            ...
            ItemComposition itemComposition = CupidBot.getClient().getItemDefinition(item.getId());
    ...
    public static void invokeMenu(Rs2ItemModel rs2Item, String action) {
        ...
        if (!Rs2Tab.switchTo(InterfaceTab.EQUIPMENT)) {
            CupidBot.log("Failed to switch to equipment tab", Level.ERROR);
            return;
        }
        ...
        if (action.equalsIgnoreCase("remove")) {
            ...
            if (identifier == -1) {
                ...
                if (subMenuEntry == null) {
                    CupidBot.log("Item=" + rs2Item.getName() + " does not have a subaction=" + action, Level.ERROR);
                    return;
                }
                int mainMenuIndex = actions.indexOf(subMenuEntry.getKey());
                if (mainMenuIndex < 0) {
                    CupidBot.log("Cannot find action=%s, in main actions=%s, mainMenuIndex=%s", subMenuEntry.getKey(), String.join(", ", actions), mainMenuIndex, Level.ERROR);
                    return;
                }
                target = "";
        ...
        Rectangle rectangle = new Rectangle(1, 1, CupidBot.getClient().getCanvasWidth(), CupidBot.getClient().getCanvasHeight());
    ...
    private static Rectangle getSafeBounds(int interfaceId, int childId) {
        ...
        return new Rectangle(1, 1,
                CupidBot.getClient().getCanvasWidth(),
                CupidBot.getClient().getCanvasHeight());
    }
}