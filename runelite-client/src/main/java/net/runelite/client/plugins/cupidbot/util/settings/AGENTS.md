...
2. With the client running + logged in, use the CLI:
   - `./cupidbot-cli widgets search "<keywords>"` to locate the widget
   - `./cupidbot-cli widgets describe <group> <child> --depth 3` to inspect structure
   - `./cupidbot-cli widgets invoke <group> <child> --param0 N --action <Toggle|Select>`
   - `./cupidbot-cli varbit <id>` to confirm the change landed
...
- `widget.isHidden()` must run on the client thread (`CupidBot.getClientThread().runOnClientThreadOptional(...)`).
