package net.runelite.client.plugins.cupidbot.util.walker.lifecycle;
...
public final class Rs2WalkerLifecycleRuntime {
    ...
    public static void applyWalkerDestination(WorldPoint target) {
        ...
        Player localPlayer = CupidBot.getClientThread().invoke(() -> client.getLocalPlayer());
        ...
        WorldPoint start = CupidBot.getClientThread().invoke(() -> {
            if (client.getTopLevelWorldView().isInstance()) {
                ...
                WorldPoint exitPortal = net.runelite.client.plugins.cupidbot.shortestpath.PohPanel.getExitPortalTile();
                if (exitPortal != null) {
                    CupidBot.log("[Walker] In POH instance — remapping pathfinder start " + computed
        ...
        CupidBot.getClientThread().runOnSeperateThread(() -> restartPathfinding(effectiveStart, target));
    }
