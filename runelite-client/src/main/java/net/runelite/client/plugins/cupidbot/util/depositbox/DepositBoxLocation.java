...
public enum DepositBoxLocation {
    ...
    public boolean hasRequirements() {
        boolean hasLineOfSight = CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            Player localPlayer = CupidBot.getClient().getLocalPlayer();
            if (localPlayer == null) return false;
            return localPlayer.getWorldArea().hasLineOfSightTo(CupidBot.getClient().getTopLevelWorldView(), worldPoint);
        }).orElse(false);
        switch (this) {
            case CRAFTING_GUILD:
                boolean hasFaladorHardDiary = CupidBot.getVarbitValue(Varbits.DIARY_FALADOR_HARD) == 1;
            ...
            case GEM_MINE:
                if (!isMember()) return false;
                if (hasLineOfSight) return true;
                return isMember() && CupidBot.getVarbitValue(Varbits.DIARY_KARAMJA_MEDIUM) == 1;
            case LEGENDS_GUILD:
                if (!isMember()) return false;
                if (hasLineOfSight) return true;
