package net.runelite.client.plugins.cupidbot.shortestpath;
...
import net.runelite.client.plugins.cupidbot.shortestpath.components.ComboBoxListRenderer;
import net.runelite.client.plugins.cupidbot.ui.CupidBotHotkeyButton;
...
import net.runelite.client.plugins.cupidbot.util.depositbox.DepositBoxLocation;
import net.runelite.client.plugins.cupidbot.util.depositbox.Rs2DepositBox;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Allotments;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Birds;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Bushes;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Chinchompas;
import net.runelite.client.plugins.cupidbot.util.walker.enums.CompostBins;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Farming;
import net.runelite.client.plugins.cupidbot.util.walker.enums.FruitTrees;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Herbs;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Hops;
import net.runelite.client.plugins.cupidbot.util.walker.enums.HuntingAreas;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Insects;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Kebbits;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Salamanders;
import net.runelite.client.plugins.cupidbot.util.walker.enums.SlayerMasters;
import net.runelite.client.plugins.cupidbot.util.walker.enums.SpecialHuntingAreas;
import net.runelite.client.plugins.cupidbot.util.walker.enums.Trees;
...
import net.runelite.client.plugins.cupidbot.questhelper.steps.ConditionalStep;
...
public class ShortestPathPanel extends PluginPanel
{
	...
	private JPanel createCustomLocationPanel()
	{
		...
		panel.setBorder(createCenteredTitledBorder("Travel to Custom Location", "/net/runelite/client/plugins/cupidbot/shortestpath/Map_link_icon.png"));
	...
	private JPanel createBankPanel()
	{
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		panel.setBorder(createCenteredTitledBorder("Travel to Bank", "/net/runelite/client/plugins/cupidbot/shortestpath/Bank_icon.png"));
	...
	private JPanel createDepositBoxPanel()
	{
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		panel.setBorder(createCenteredTitledBorder("Travel to DepositBox", "/net/runelite/client/plugins/cupidbot/shortestpath/Bank_icon.png"));
	...
	private JPanel createSlayerMasterPanel()
	{
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		panel.setBorder(createCenteredTitledBorder("Travel to Slayer Master", "/net/runelite/client/plugins/cupidbot/shortestpath/Slayer_Master_icon.png"));
	...
	private JPanel createQuestLocationPanel()
	{
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		panel.setBorder(createCenteredTitledBorder("Travel to Quest Location", "/net/runelite/client/plugins/cupidbot/questhelper/quest_icon.png"));
		...
		startButton.addActionListener(e -> {
			...
			{
				CupidBot.log("Walking to quest objective location");
			...
			else
			{
				...
				if (qhp == null)
				{
					CupidBot.log("Cannot walk to quest location: QuestHelper plugin not enabled");
				}
				else if (qhp.getSelectedQuest() == null)
				{
					CupidBot.log("Cannot walk to quest location: No quest selected in QuestHelper");
				}
				else
				{
					CupidBot.log("Cannot walk to quest location: Current quest step has no location");
				}
			}
		});
	...
	private JPanel createFarmingPanel()
	{
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		panel.setBorder(createCenteredTitledBorder("Travel to Farming Location", "/net/runelite/client/plugins/cupidbot/shortestpath/Farming_patch_icon.png"));
	...
	private JPanel createHunterCreaturePanel()
	{
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		panel.setBorder(createCenteredTitledBorder("Travel to Hunter Creature", "/net/runelite/client/plugins/cupidbot/shortestpath/Hunter_icon.png"));
	 ...
	 */
	private CupidBotHotkeyButton createHotkeyButton(String keyName, Keybind initial, String tooltip)
	{
		CupidBotHotkeyButton button = new CupidBotHotkeyButton(initial == null ? Keybind.NOT_SET : initial, false);
	...
	{
		CupidBot.log("Web walking starting. Traveling to Custom Location (" + point.getX() + ", " + point.getY() + ", " + point.getPlane() + ").");
		plugin.getShortestPathScript().setTriggerWalker(point);
	}

	void stopWalking()
	{
		CupidBot.log("Web walking stopping..");
	...
	void startWalkingNearestBank()
	{
		CompletableFuture.supplyAsync(Rs2Bank::getNearestBank)
			.thenAccept(nearestBank -> {
				...
				{
					CupidBot.log("WebWalker: could not find a nearest bank.");
				}
			})
			.exceptionally(ex -> {
				CupidBot.log("Error while finding the nearest bank: " + ex.getMessage());
				return null;
			});
	}
	...
	void startWalkingNearestDepositBox()
	{
		CompletableFuture.supplyAsync(Rs2DepositBox::getNearestDepositBox)
			.thenAccept(nearestDepositBox -> {
				...
				{
					CupidBot.log("WebWalker: could not find a nearest deposit box.");
				}
			})
			.exceptionally(ex -> {
				CupidBot.log("Error while finding the nearest deposit box: " + ex.getMessage());
				return null;
			});
	}
	...
	{
		return (QuestHelperPlugin) CupidBot.getPluginManager().getPlugins().stream()
	...
	WorldPoint getCurrentQuestLocation()
	{
		...
		{
			CupidBot.log("Error getting quest location: " + e.getMessage());
		}
	...
	private JPanel createClueLocationPanel()
	{
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		panel.setBorder(createCenteredTitledBorder("Travel to Clue Location", "/net/runelite/client/plugins/cupidbot/shortestpath/Clue_scroll_icon.png"));
		...
		startButton.addActionListener(e -> {
			...
			{
				CupidBot.log("Walking to clue scroll location");
			...
			else
			{
				...
				if (cluePlugin == null)
				{
					CupidBot.log("Cannot walk to clue location: ClueScroll plugin not enabled");
				}
				else if (cluePlugin.getClue() == null)
				{
					CupidBot.log("Cannot walk to clue location: No active clue scroll");
				}
				else
				{
					CupidBot.log("Cannot walk to clue location: Current clue has no location");
				}
			}
		});
	...
	{
		return (ClueScrollPlugin) CupidBot.getPluginManager().getPlugins().stream()
