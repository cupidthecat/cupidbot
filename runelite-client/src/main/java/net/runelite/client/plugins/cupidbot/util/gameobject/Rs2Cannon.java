...
public class Rs2Cannon {
    ...
    public static boolean repair() {
        ...
        CupidBot.status = "Repairing Cannon";
    ...
    public static boolean refill(int cannonRefillAmount) {
        ...
        int cannonBallsLeft = CupidBot.getClientThread().runOnClientThreadOptional(() -> CupidBot.getClient().getVarpValue(VarPlayer.CANNON_AMMO)).orElse(0);

        if (cannonBallsLeft > cannonRefillAmount) return false;

        CupidBot.status = "Refilling Cannon";
        ...
        sleep(1200);
        Rs2GameObject.interact(cannon, "Fire");
        sleepUntil(() -> CupidBot.getClientThread().runOnClientThreadOptional(() -> CupidBot.getClient().getVarpValue(VarPlayer.CANNON_AMMO)).orElse(0) > Rs2Random.between(10, 15));
