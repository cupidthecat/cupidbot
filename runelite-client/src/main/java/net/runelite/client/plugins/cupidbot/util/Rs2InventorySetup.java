package net.runelite.client.plugins.cupidbot.util;
...
import net.runelite.client.plugins.cupidbot.inventorysetups.InventorySetupsItem;
import net.runelite.client.plugins.cupidbot.inventorysetups.InventorySetupsStackCompareID;
...
/**
 * Utility class for managing inventory setups in the CupidBot plugin.
...
public class Rs2InventorySetup {
    ...
    /**
     * When {@code -Dcupidbot.bank.validateInventorySetup=true}, {@link #loadInventory()} logs warnings for stale preset
     * ids, id/name drift, and impossible non-stackable quantities on a single row.
     */
    private static final String PROP_VALIDATE_INVENTORY_SETUP = "cupidbot.bank.validateInventorySetup";
    ...
    public Rs2InventorySetup(InventorySetup setup, ScheduledFuture<?> mainScheduler) {
        ...
        _mainScheduler = mainScheduler;
        if (inventorySetup == null) {
            CupidBot.showMessage("Inventory load error!", 10);
	...
	/**
	 * Level-aware structured lines: {@code [InventorySetup:<preset>] message} via {@link CupidBot#log(Level, String, Object...)}.
	 */
	private void logSetup(Level level, String format, Object... args) {
		...
		if (args == null || args.length == 0) {
			CupidBot.log(prefix + format, level);
		} else {
			CupidBot.log(level, prefix + format, args);
		}
	}
    ...
    public boolean hasSpellBook() {
        ...
        return setupBook == CupidBot.getVarbitValue(VarbitID.SPELLBOOK);
    }
