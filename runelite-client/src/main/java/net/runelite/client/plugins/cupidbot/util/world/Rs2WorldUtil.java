package net.runelite.client.plugins.cupidbot.util.world;
 ...
 * population and ping analysis, and intelligent world hopping for the CupidBot framework.
 * <p>
 * Features:
 * <ul>
 ...
 *
 *     <li>Integrates with CupidBot's login and break handler systems for seamless automation</li>
...
public class Rs2WorldUtil {
    ...
    public static boolean canAccessWorld(int worldId) {
        try {
            ...
            if(CupidBot.getClient() != null &&  CupidBot.getClient().getLocalPlayer() != null){
                boolean isInSeasonalWorld = CupidBot.getClient().getWorldType().contains(net.runelite.api.WorldType.SEASONAL);
    ...
    public static boolean hopWorld(CompletableFuture<?> scheduledFuture, int world, Integer currentAttempts, WorldHoppingConfig worldHoppingConfig) {
        try {
            CupidBot.status = "Stock level inadequate - hopping worlds with smart retry logic";
            Rs2Shop.closeShop();
            Rs2Bank.closeBank();
            ...
            while (attemptsPerWorld < maxAttemptsPerWorld) {
               ...
               ;
                sleepUntil(() -> CupidBot.hopToWorld(world), 6000);
                sleepUntil(() -> !CupidBot.isHopping(), 6000);
                if (CupidBot.isHopping()) {
                    CupidBot.status = "Failed to hop to world " + world + " (attempt " + attemptsPerWorld + ")";
                ...
                sleepUntil(() -> CupidBot.getClient().getGameState() == GameState.LOGGED_IN, 6000);
                if ( Rs2Player.getWorld() != world){
                    CupidBot.status = "World hop to " + world + " failed (current world: " + Rs2Player.getWorld() + ")";
                ...
                CupidBot.status = "Successfully hopped to world " + world + " (attempt " + attemptsPerWorld + ")";
            ...
            CupidBot.status = "Failed to hop worlds after " + attemptsPerWorld  + " attempts";
            return false;

        } catch (Exception e) {
            CupidBot.logStackTrace(".hopWorld", e);
            return false;
        }
    }
    ...
    public static boolean hopToNextBestWorld(CompletableFuture<?> scheduledFuture,
        ...
        try {
            CupidBot.status = "Finding next best world for hopping";
            ...
            if (CupidBot.isHopping()) {
                ...
                sleepUntil(() -> !CupidBot.isHopping(), 10000);
                return !CupidBot.isHopping(); // Return true if hopping completed
            }
            ...
            CupidBot.logStackTrace("LocationRequirementUtil.hopToNextBestWorld", e);
            return false;
        }
    }
    ...
    private static boolean waitForWorldServiceReady() {
        try {
            return sleepUntil(() -> {
                WorldResult result = CupidBot.getWorldService().getWorlds();
    ...
    public static boolean performLogin(boolean useRandomWorld, WorldRegion preferredRegion,
        ...
        try {
            ...
            boolean loginSuccess = sleepUntil(() -> CupidBot.isLoggedIn(), 10000);
