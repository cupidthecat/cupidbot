package net.runelite.client.plugins.cupidbot.util.leaguetransport;
...
final class LeaguesTransportTeleport
{
	...
	private static boolean isClientReadyForCalibration(Client client)
	{
		...
		{
			net.runelite.client.callback.ClientThread clientThread = CupidBot.getClientThread();
		...
		SwingUtilities.invokeLater(() ->
		{
			try
			{
				...
				int res = JOptionPane.showConfirmDialog(
						...
						"CupidBot: Calibrate Leagues teleports",
			...
			try
			{
				...
				try
				{
					JOptionPane.showMessageDialog(
							null,
							msg,
							"CupidBot: Leagues calibration complete",
							JOptionPane.INFORMATION_MESSAGE);
				}
				catch (Exception e)
				{
			...
			boolean logNoOpWhenFullyCalibrated)
	{
		...
		}, "cupidbot-leagues-landing-calibration");
		t.setDaemon(true);
	...
	{
		Optional<EnumSet<LeaguesRegion>> unlockedOpt = CupidBot.getClientThread().runOnClientThreadOptional(() ->
		{
	...
	static LeaguesTeleportResult leaguesTeleport(LeaguesRegion region, int timeoutMs)
	{
		...
		{
			CupidBot.status = gate.contextFailureMessage;
			return LeaguesTeleportResult.failure(
	...
	{
		var ct = CupidBot.getClientThread();
		if (ct == null)
		{
			return;
		}
		...
		public void tick()
		{
			...
			switch (step)
			{
				case 0:
					...
					CupidBot.status = "Leagues: open activities";
					invokeCcOp(LeagueTransportWidgets.pack(LeagueTransportWidgets.ACTIVITIES_GROUP, LeagueTransportWidgets.ACTIVITIES_CHILD), "Leagues", "");
					step = 1;
					return;
				case 1:
					...
					CupidBot.status = "Leagues: open leagues";
					invokeCcOp(LeagueTransportWidgets.pack(LeagueTransportWidgets.LEAGUES_GROUP, LeagueTransportWidgets.LEAGUES_CHILD), "Leagues", "");
					step = 2;
					return;
				case 2:
					...
					CupidBot.status = "Leagues: open areas";
				...
				case 3:
				{
					...
					{
						CupidBot.status = "Leagues: teleport " + targetRegion.getDisplayName();
					...
					{
						CupidBot.status = "Leagues: areas menu shield";
						invokeCcOp(LeagueTransportWidgets.pack(shield.getGroup(), shield.getChild()), shield.getCcOpOption(), shield.getCcOpTarget());
	...
	private static EnumSet<LeaguesRegion> readUnlockedRegionsFromSelectionVarbits()
	{
		...
		{
			int areaId = CupidBot.getVarbitValue(vb);
	...
	static String leaguesContextRejectOrEmptySuccess()
	{
		...
		if (CupidBot.getVarbitValue(VarbitID.LEAGUE_ACCOUNT) <= 0)
		{
	...
	{
		Optional<String> msgOpt = CupidBot.getClientThread().runOnClientThreadOptional(LeaguesTransportTeleport::leaguesContextRejectOrEmptySuccess);
		if (!msgOpt.isPresent())
		{
	...
	{
		return CupidBot.getVarbitValue(VarbitID.LEAGUE_TYPE) > 0;
	}
	...
	private static void invokeCcOp(int packedWidgetId, String option, String target)
	{
		...
		Rectangle bounds = CupidBot.getClientThread().runOnClientThreadOptional(() ->
		{
