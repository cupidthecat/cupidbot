package net.runelite.client.plugins.cupidbot.configs;
...
import net.runelite.client.plugins.cupidbot.util.misc.SpecialAttackWeaponEnum;
...
public class SpecialAttackConfigs {
    ...
    public boolean useSpecWeapon() {
        ...
        if (!CupidBot.getSpecialAttackConfigs().isUseSpecialAttack()) return false;
        if (Rs2Combat.getSpecEnergy() < CupidBot.getSpecialAttackConfigs().getMinimumSpecEnergy() && currentEquipment == null) return false;
        ...
        for (SpecialAttackWeaponEnum specialAttackWeapon :SpecialAttackWeaponEnum.values()) {
            boolean hasPresetSpecialAttackWeapon = Rs2Inventory.hasItem(specialAttackWeapon.getName())
                    && CupidBot.getSpecialAttackConfigs().getSpecialAttackWeapon() != null
                    && CupidBot.getSpecialAttackConfigs().getSpecialAttackWeapon().getName().toLowerCase().contains(specialAttackWeapon.getName());
            boolean hasRandomSpecialAttackWeapon = Rs2Inventory.hasItem(specialAttackWeapon.getName()) && CupidBot.getSpecialAttackConfigs().getSpecialAttackWeapon() == null;
        ...
        for (SpecialAttackWeaponEnum specialAttackWeapon :SpecialAttackWeaponEnum.values()) {
            boolean hasPresetSpecialAttackWeapon = Rs2Equipment.isWearing(specialAttackWeapon.getName())
                    && CupidBot.getSpecialAttackConfigs().getSpecialAttackWeapon() != null
                    && CupidBot.getSpecialAttackConfigs().getSpecialAttackWeapon().getName().toLowerCase().contains(specialAttackWeapon.getName());
            boolean hasRandomSpecialAttackWeapon = Rs2Equipment.isWearing(specialAttackWeapon.getName()) && CupidBot.getSpecialAttackConfigs().getSpecialAttackWeapon() == null;
            if (hasPresetSpecialAttackWeapon || hasRandomSpecialAttackWeapon) {
                if (useSpecWeapon(specialAttackWeapon.getName(), specialAttackWeapon.getEnergyRequired(), specialAttackWeapon.is2H()))
                    return true;
            }
