...
import static net.runelite.client.plugins.cupidbot.util.widget.Rs2Widget.*;
...
public class Rs2SpellBookSettings {
    ...
    public static boolean toggleSpellFilter(String text, boolean toggle, int varbitId, boolean closeInterface) {
        ...
        if (CupidBot.getVarbitValue(varbitId) == expectedValue) {
            return true;
        }
        ...
        sleepUntil(() -> CupidBot.getVarbitValue(varbitId) == expectedValue, 2000);

        if (closeInterface) {
            closeSpellBookFilter();
        }

        return CupidBot.getVarbitValue(varbitId) == expectedValue;
    }
