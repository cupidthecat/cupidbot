package net.runelite.client.plugins.cupidbot.util.poh.data;
...
public enum NexusPortal implements PohTeleport {
    ...
    public static List<NexusPortal> getAvailableTeleports() {
        ...
        for (int varbit : VARBITS) {
            int value = CupidBot.getVarbitValue(varbit);
            if (value <= 0) continue;

            if (value == 1) {
                teleports.add(NexusPortal.VARROCK);
                if (CupidBot.getVarbitValue(VARROCK_DIARY_MEDIUM_COMPLETE) == 1) {
