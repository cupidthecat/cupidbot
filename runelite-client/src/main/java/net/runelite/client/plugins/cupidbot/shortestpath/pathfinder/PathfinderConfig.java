...
import net.runelite.client.plugins.cupidbot.shortestpath.pathfinder.policy.TransportRequirementPolicy;
...
import net.runelite.client.plugins.cupidbot.util.magic.RuneFilter;
...
import net.runelite.client.plugins.cupidbot.util.walker.WebWalkLog;
...
import static net.runelite.client.plugins.cupidbot.shortestpath.TransportType.TELEPORTATION_ITEM;
import static net.runelite.client.plugins.cupidbot.shortestpath.TransportType.TELEPORTATION_SPELL;
...
public class PathfinderConfig {
    ...
    private void refreshTransports(WorldPoint target) {
        useFairyRings = ShortestPathPlugin.override("useFairyRings", config.useFairyRings())
                ...
                || CupidBot.getVarbitValue(VarbitID.LUMBRIDGE_DIARY_ELITE_COMPLETE) == 1);
        ...
        CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            ...
            for (int id : varbitIds) {
                CupidBot.getVarbitValue(id);
            }
            for (int id : varplayerIds) {
                CupidBot.getVarbitPlayerValue(id);
            }
            return true;
        });
    ...
    public boolean isDangerousAdjacentTile(int packedPoint) {
        return isDangerousAdjacentTilePacked(packedPoint);
    }

    static boolean isDangerousAdjacentTilePacked(int packedPoint) {
    ...
    private void refreshRestrictionData() {
        ...
        allRestrictions.stream()
                .filter(entry -> {
                    ...
                    // Varbit check
                    if (entry.getVarbits().stream().anyMatch(varbitCheck -> !varbitCheck.matches(CupidBot.getVarbitValue(varbitCheck.getVarbitId())))) {
                        return true;
                    }
                    // Varplayer check
    ...
    private boolean hasRequiredLevels(Transport transport) {
        ...
        return IntStream.range(0, requiredLevels.length)
            ...
            .allMatch(i -> CupidBot.getClient().getBoostedSkillLevel(SKILLS[i]) >= requiredLevels[i]);
    }
...
//        return Rs2Magic.quickCanCast(displayInfo);
    ...
    private boolean hasChronicleCharges() {
        ...
        String charges = CupidBot.getConfigManager()
        ...
        if (charges == null || charges.isEmpty()) {
            ...
            charges = CupidBot.getConfigManager().getRSProfileConfiguration(ItemChargeConfig.GROUP, ItemChargeConfig.KEY_CHRONICLE);
        }
    ...
    private int computeTransportRefreshCacheKeyHash(WorldPoint target, Rs2LeaguesTransport.LeaguesContext leaguesCtx) {
        ...
        return Objects.hash(
                ...
                CupidBot.getVarbitValue(VarbitID.LEAGUE_TYPE),
                leaguesCtx.isActive(),
    ...
    static int computeTransportRefreshVerificationHash(int[] boostedLevels, int[] sortedVarbits, int[] sortedVarplayers,
        ...
        for (int id : sortedVarbits) {
            h = 31 * h + id;
            h = 31 * h + CupidBot.getVarbitValue(id);
        }
        for (int id : sortedVarplayers) {
            h = 31 * h + id;
            h = 31 * h + CupidBot.getVarbitPlayerValue(id);
        }
