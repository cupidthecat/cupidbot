...
import net.runelite.client.plugins.cupidbot.pouch.Pouch;
...
public class Rs2Inventory {
    ...
    public static ItemContainer inventory() {
        return CupidBot.getClient().getItemContainer(InventoryID.INV);
    }
    ...
    public static Stream<Rs2ItemModel> items() {
        if (inventoryItems.isEmpty() && CupidBot.isLoggedIn()) {
            CupidBot.getClientThread().runOnClientThreadOptional(() -> {
                final ItemContainer itemContainer = CupidBot.getClient().getItemContainer(InventoryID.INV);
    ...
    public static boolean combine(Rs2ItemModel primary, Rs2ItemModel secondary) {
        ...
        if (primaryItem == null) {
            CupidBot.log("Primary item not found in the inventory.");
            return false;
        }
        ...
        if (secondaryItem == null) {
            CupidBot.log("No valid secondary item found to combine with.");
            return false;
        }
    ...
    public static boolean dropAllExcept(int gpValue, String[] ignoreItems) {
        ...
        final Map<Integer, Long> priceMap = CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            Map<Integer, Long> map = new HashMap<>();
            for (Rs2ItemModel item : inventorySnapshot) {
                map.put(item.getSlot(), (long) CupidBot.getItemManager().getItemPrice(item.getId()) * item.getQuantity());
            }
            return map;
    ...
    private static <T> T getSelectedValueOrDefault(Function<Widget, T> valueSupplier, T _default) {
        final Widget widget = CupidBot.getClient().getSelectedWidget();
    ...
    private static void invokeMenu(Rs2ItemModel rs2Item, String action) {
        ...
        CupidBot.status = action + " " + rs2Item.getName();
        ...
        boolean isDepositBoxOpen = !CupidBot.getClientThread().runOnClientThreadOptional(() -> Rs2Widget.getWidget(ComponentID.DEPOSIT_BOX_INVENTORY_ITEM_CONTAINER) == null
  ...
  /*      if (identifier > 5) {
    ...
    public static Widget getInventory() {
        ...
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            for (int id : new int[] {ComponentID.INVENTORY_CONTAINER, ComponentID.BANK_INVENTORY_ITEM_CONTAINER,
                ...
                final Widget widget = CupidBot.getClient().getWidget(id);
    ...
    public static boolean sellItem(String itemName, String quantity) {
        ...
        if (item == null) {
            CupidBot.log("Item not found in inventory.");
            return false;
        }
    ...
    public static boolean moveItemToSlot(Rs2ItemModel item, int slot) {
        ...
        if (inventory == null) {
            ...
            if (inventory == null) {
                CupidBot.log("Inventory widget is null", Level.ERROR);
                return false;
            }
        }
        ...
        CupidBot.drag(itemBounds, slotBounds);
