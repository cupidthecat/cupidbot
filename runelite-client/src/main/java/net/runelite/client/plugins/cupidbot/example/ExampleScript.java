package net.runelite.client.plugins.cupidbot.example;
...
public class ExampleScript extends Script {
    ...
    public boolean run() {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                log.info("========================================");
                log.info("  CUPIDBOT POST-MERGE SMOKE TEST");
                log.info("========================================");
    ...
    private void checkClientState() {
        ...
        check("CupidBot.isLoggedIn()", () -> {
            assertTrue(CupidBot.isLoggedIn(), "expected logged in");
        });

        check("Client.getLocalPlayer() not null", () -> {
            assertNotNull(CupidBot.getClient().getLocalPlayer(), "local player is null");
        });

        check("GameState is LOGGED_IN", () -> {
            GameState state = CupidBot.getClient().getGameState();
    ...
    private void checkWorldViewAndThreading() {
        ...
        check("TopLevelWorldView not null", () -> {
            WorldView wv = CupidBot.getClient().getTopLevelWorldView();
        ...
        check("Player WorldView is top level", () -> {
            ...
            WorldView wv = CupidBot.getClientThread().invoke(() -> player.getWorldView());
        ...
        check("ClientThread.invoke() works", () -> {
            String result = CupidBot.getClientThread().invoke(() -> "ok");
    ...
    private void checkNpcCache() {
        ...
        check("Rs2NpcCache.getStream() not null", () -> {
            Stream<Rs2NpcModel> stream = CupidBot.getRs2NpcCache().getStream();
            assertNotNull(stream, "npc stream is null");
        });

        check("Rs2NpcCache stream has entries", () -> {
            long count = CupidBot.getRs2NpcCache().getStream().count();
            assertTrue(count > 0, "no NPCs in cache (are there NPCs nearby?)");
        });

        check("First NPC has valid world location", () -> {
            Rs2NpcModel npc = CupidBot.getRs2NpcCache().getStream().findFirst().orElse(null);
        ...
        check("Rs2NpcCache.query().nearest() works", () -> {
            CupidBot.getRs2NpcCache().query().nearest();
        });
    }
    ...
    private void checkTileObjectCache() {
        ...
        check("Rs2TileObjectCache.getStream() not null", () -> {
            Stream<Rs2TileObjectModel> stream = CupidBot.getRs2TileObjectCache().getStream();
            assertNotNull(stream, "tile object stream is null");
        });

        check("Rs2TileObjectCache stream has entries", () -> {
            long count = CupidBot.getRs2TileObjectCache().getStream().count();
            assertTrue(count > 0, "no tile objects in cache");
        });

        check("First TileObject has valid ID", () -> {
            Rs2TileObjectModel obj = CupidBot.getRs2TileObjectCache().getStream().findFirst().orElse(null);
        ...
        check("Rs2TileObjectCache.query().nearest() works", () -> {
            CupidBot.getRs2TileObjectCache().query().nearest();
        });
    }
    ...
    private void checkGroundItemCache() {
        ...
        check("Rs2TileItemCache.getStream() callable", () -> {
            CupidBot.getRs2TileItemCache().getStream();
        });
    }
    ...
    private void checkPlayerCache() {
        ...
        check("Rs2PlayerCache.getStream() callable", () -> {
            CupidBot.getRs2PlayerCache().getStream();
        });
    }
    ...
    private void checkEquipment() {
        ...
        check("Rs2Equipment.equipment() callable", () -> {
            CupidBot.getClientThread().invoke(() -> Rs2Equipment.equipment());
        });
