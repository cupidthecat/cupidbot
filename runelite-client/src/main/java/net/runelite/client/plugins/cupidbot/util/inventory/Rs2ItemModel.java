...
public class Rs2ItemModel {
    ...
    public Rs2ItemModel(int id, int quantity, int slot) {
        ...
        ItemComposition itemComposition = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                CupidBot.getClient().getItemDefinition(id)).orElse(null);
        if (itemComposition == null) {
            initializeDefaults();
        } else {
            initializeFromComposition(itemComposition);
    ...
    private void ensureCompositionLoaded() {
        if (itemComposition == null && id > 0) {
            CupidBot.getClientThread().runOnClientThreadOptional(() -> CupidBot.getClient().getItemDefinition(id)).ifPresent(this::initializeFromComposition);
        }
    }
    ...
    public int getPrice() {
        return CupidBot.getClientThread().runOnClientThreadOptional(() ->
                CupidBot.getItemManager().getItemPrice(id) * quantity).orElse(0);
    }
    ...
    public boolean isHaProfitable() {
        int natureRunePrice = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                CupidBot.getItemManager().getItemPrice(ItemID.NATURERUNE)).orElse(0);
    ...
    public static int getNotedId(int itemId) {
        ItemComposition composition = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                CupidBot.getClient().getItemDefinition(itemId)
        ).orElse(null);
    ...
    private void initializeFromComposition(ItemComposition itemComposition) {
        ...
        if (this.isNoted) {
            CupidBot.getClientThread().runOnClientThreadOptional(() ->
                    CupidBot.getClient().getItemDefinition(itemComposition.getLinkedNoteId())
        ...
        // This has to be run in cupidbot's client thread
        this.subops = CupidBot.getClientThread().runOnClientThreadOptional(itemComposition::getSubops).orElse(null);
