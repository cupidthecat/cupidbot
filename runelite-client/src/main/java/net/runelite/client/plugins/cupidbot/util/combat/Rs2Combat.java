package net.runelite.client.plugins.cupidbot.util.combat;
...
import net.runelite.client.plugins.cupidbot.util.combat.weapons.*;
...
import net.runelite.client.plugins.cupidbot.util.magic.Rs2CombatSpells;
...
public class Rs2Combat {
    ...
    public static boolean setAttackStyle(WidgetInfo style) {
        Widget widget = CupidBot.getClient().getWidget(style);
...
//        }
    ...
    public static boolean setAutoCastSpell(Rs2CombatSpells combatSpell, boolean useDefensiveCast) {
        ...
        if (Rs2Magic.getCurrentAutoCastSpell() == combatSpell && CupidBot.getVarbitValue(Varbits.DEFENSIVE_CASTING_MODE) == (useDefensiveCast ? 1 : 0)) return true;
        ...
        return sleepUntilTrue(() -> Rs2Magic.getCurrentAutoCastSpell() == combatSpell && CupidBot.getVarbitValue(Varbits.DEFENSIVE_CASTING_MODE) == (useDefensiveCast ? 1 : 0));
    }
    ...
    public static boolean setAutoRetaliate(boolean enable) {
        ...
        if (CupidBot.getVarbitPlayerValue(VarPlayerID.OPTION_NODEF) == expectedVarPlayerValue) return true;
        ...
        return sleepUntil(() -> CupidBot.getVarbitPlayerValue(VarPlayerID.OPTION_NODEF) == expectedVarPlayerValue, 2_000);
    }
    ...
    public static boolean setSpecState(boolean state, int specialAttackEnergyRequired) {
        int currentSpecEnergy = CupidBot.getClientThread().runOnClientThreadOptional(
                () -> CupidBot.getClient().getVarpValue(VarPlayer.SPECIAL_ATTACK_PERCENT)
        ).orElse(0);
        ...
        CupidBot.getMouse().click(Rs2Widget.getWidget(10485795).getBounds());

        log("Used special attack");

       //  CupidBot.doInvoke(new NewMenuEntry(-1, 10485795, MenuAction.CC_OP.getId(), 1, -1, "Special Attack"), new Rectangle(1, 1, CupidBot.getClient().getCanvasWidth(), CupidBot.getClient().getCanvasHeight()));
    ...
    public static int getSpecEnergy() {
        return CupidBot.getVarbitPlayerValue(VarPlayerID.SA_ENERGY);
    }
    ...
    public static boolean getSpecState() {
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            Widget widget = CupidBot.getClient().getWidget(WidgetInfo.MINIMAP_SPEC_ORB.getId() + 4);
            if (widget == null) return false;
    ...
    public static boolean inCombat() {
        if (!CupidBot.isLoggedIn()) return false;
    ...
    public static String getWeaponAttackStyle() {
        final int attackStyleVarbit = CupidBot.getVarbitPlayerValue(VarPlayerID.COM_MODE);
        final int weaponTypeVarbit = CupidBot.getVarbitValue(VarbitID.COMBAT_WEAPON_CATEGORY);
        int weaponStyleEnum = CupidBot.getEnum(EnumID.WEAPON_STYLES).getIntValue(weaponTypeVarbit);
        int[] weaponStyleStructs = CupidBot.getEnum(weaponStyleEnum).getIntVals();
        StructComposition attackStylesStruct = CupidBot.getStructComposition(weaponStyleStructs[attackStyleVarbit]);
