...
@Slf4j
public class CupidBotProfilePanel extends CupidBotPluginPanel {
    ...
    private static final ImageIcon DELETE_ICON = new ImageIcon(ImageUtil.loadImageResource(CupidBotProfilePanel.class, "mdi_delete.png"));
    private static final ImageIcon EXPORT_ICON = new ImageIcon(ImageUtil.loadImageResource(CupidBotProfilePanel.class, "mdi_export.png"));
    private static final ImageIcon RENAME_ICON;
    private static final ImageIcon RENAME_ACTIVE_ICON;
    private static final ImageIcon CLONE_ICON = new ImageIcon(ImageUtil.loadImageResource(CupidBotProfilePanel.class, "mdi_content-duplicate.png"));
    private static final ImageIcon LINK_ICON;
    private static final ImageIcon LINK_ACTIVE_ICON;
    private static final ImageIcon ARROW_RIGHT_ICON = new ImageIcon(ImageUtil.loadImageResource(CupidBotProfilePanel.class, "/util/arrow_right.png"));
    ...
    @Getter
    private Map<Long, CupidBotProfilePanel.ProfileCard> cards = new HashMap<>();
    ...
    private JPanel createHeaderPanel() {
        ...
        JLabel subtitleLabel = new JLabel("Manage your cupidbot profiles");
    ...
    private void onProfileChanged(ProfileChanged ev) {
        ...
        SwingUtilities.invokeLater(() -> {
            for (CupidBotProfilePanel.ProfileCard card : cards.values()) {
                card.setActive(false);
            }

            CupidBotProfilePanel.ProfileCard card = cards.get(configManager.getProfile().getId());
            if (card == null) {
                reload();
                return;
            }
    ...
    private void reload(List<ConfigProfile> profiles) {
        SwingUtilities.invokeLater(() -> {
            ...
            Map<Long, CupidBotProfilePanel.ProfileCard> prevCards = cards;
            cards = new HashMap<>();
            ...
            for (ConfigProfile profile : profiles) {
                ...
                CupidBotProfilePanel.ProfileCard prev = prevCards.get(profile.getId());
                final long id = profile.getId();
                final List<String> defaultForRsProfiles = profile.getDefaultForRsProfiles();
                CupidBotProfilePanel.ProfileCard pc = new CupidBotProfilePanel.ProfileCard(
                        profile,
                        activePanel == id,
    ...
    private class ProfileCard extends JPanel {
        ...
        private ProfileCard(ConfigProfile profile, boolean isActive, boolean rsProfileDefault, boolean limited, CupidBotProfilePanel.ProfileCard prev) {
            ...
            delete.addActionListener(ev -> {
                int confirm = JOptionPane.showConfirmDialog(CupidBotProfilePanel.ProfileCard.this,
        ...
        private void stopRenamingPassword(boolean save) {
            ...
            try {
                configManager.setPassword(profile, net.runelite.client.plugins.cupidbot.util.security.Encryption.encrypt(password.getText()));
            } catch (Exception e) {
        ...
        private void stopRenamingBankPin(boolean save) {
            ...
            try {
                if (bankPin.getText().isBlank()) {
                    ...
                    configManager.setBankPin(profile, net.runelite.client.plugins.cupidbot.util.security.Encryption.encrypt(bankPin.getText()));
                }
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
    ...
    private void handleDrag(Component component) {
        CupidBotProfilePanel.ProfileCard c = (CupidBotProfilePanel.ProfileCard) component;
        ...
        try (ProfileManager.Lock lock = profileManager.lock()) {
            ...
            profiles.sort(Comparator.comparing(p -> {
                ...
                for (int idx = 0; idx < components.length; ++idx) {
                    CupidBotProfilePanel.ProfileCard card = (CupidBotProfilePanel.ProfileCard) components[idx];
