package net.runelite.client.plugins.cupidbot.util.gameobject;
...
public class Rs2GameObject {
    ...
    public static List<Tile> getTiles(int maxTileDistance) {
        ...
        Player player = CupidBot.getClient().getLocalPlayer();
        Scene scene = CupidBot.getClient().getScene();
        Tile[][][] tiles = scene.getTiles();

        int z = CupidBot.getClient().getPlane();
    ...
    public static TileObject getTileObject(Predicate<TileObject> predicate, WorldPoint anchor, int distance) {
        ...
        if (anchorLocal == null) {
            ...
            if (CupidBot.getClient() != null && CupidBot.getClient().getLocalPlayer() != null) {
                anchorLocal = CupidBot.getClient().getLocalPlayer().getLocalLocation();
            }
    ...
    public static List<TileObject> getTileObjects(Predicate<TileObject> predicate, WorldPoint anchor, int distance) {
        LocalPoint anchorLocal = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), anchor);
        if (anchorLocal == null) {
    ...
    public static List<GameObject> getGameObjects(Predicate<GameObject> predicate, WorldPoint anchor, int distance) {
        if (anchor == null || CupidBot.getClient() == null || CupidBot.getClient().getTopLevelWorldView() == null) {
            return Collections.emptyList();
        }
    ...
    private static <T extends TileObject> Stream<T> getSceneObjects(Function<Tile, Collection<? extends T>> extractor) {
        var triple = CupidBot.getClientThread().invoke(() -> {
            Player player = CupidBot.getClient().getLocalPlayer();
	...
	private static LocalPoint localPointFromWorldSafe(WorldPoint anchor) {
		WorldView worldView = CupidBot.getRs2PlayerStateCache().getLocalPlayerWorldView();
		if (worldView == null) {
			return null;
		}
    ...
    private static ObjectComposition convertToObjectCompositionInternal(int objectId, boolean ignoreImpostor) {
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            ObjectComposition comp = CupidBot.getClient().getObjectDefinition(objectId);
    ...
    public static boolean clickObject(TileObject object, String action) {
        ...
        LocalPoint playerLocal = CupidBot.getClient().getLocalPlayer() != null
                ? CupidBot.getClient().getLocalPlayer().getLocalLocation() : null;
        ...
        if (tooFar) {
            CupidBot.log("Object with id " + object.getId() + " is not close enough to interact with. Walking to the object....");
        ...
        try {
            ...
            CupidBot.status = action + " " + objComp.getName();
            ...
            if (index == -1) {
                CupidBot.log("Failed to interact with object " + object.getId() + " " + action);
            }
...
/*            if (object.getWorldView().getId() != -1) {
            ...
            if (!object.getWorldView().isTopLevel()) {
                var worldView =CupidBot.getClientThread().invoke(() ->  CupidBot.getClient().getLocalPlayer().getWorldView());
                if (worldView == null) {
                    worldViewId = CupidBot.getClient().getTopLevelWorldView().getId();
                } else {
...
// MenuEntryImpl(getOption=Use, getTarget=Barrier, getIdentifier=43700, getType=GAME_OBJECT_THIRD_OPTION, getParam0=53, getParam1=51, getItemId=-1, isForceLeftClick=true, getWorldViewId=-1, isDeprioritized=false)
            ...
            CupidBot.log("Failed to interact with object " + ex.getMessage());
        }
    ...
    public static boolean hasLineOfSight(WorldPoint point, TileObject tileObject) {
        ...
        if (tileObject instanceof GameObject) {
            ...
            WorldPoint worldPoint = WorldPoint.fromScene(CupidBot.getClient(), gameObject.getSceneMinLocation().getX(), gameObject.getSceneMinLocation().getY(), gameObject.getPlane());
            return new WorldArea(
                    worldPoint,
                    gameObject.sizeX(),
                    gameObject.sizeY())
                    .hasLineOfSightTo(CupidBot.getClient().getTopLevelWorldView(), point.toWorldArea());
        } else {
            return new WorldArea(
                    tileObject.getWorldLocation(),
                    2,
                    2)
                    .hasLineOfSightTo(CupidBot.getClient().getTopLevelWorldView(), new WorldArea(point.getX(),
    ...
    public static List<Integer> getObjectIdsByName(String name) {
        return NAME_TO_IDS_CACHE.computeIfAbsent(name, k -> {
            ...
            Class<?>[] classesToScan = {
                    ...
                    net.runelite.client.plugins.cupidbot.util.gameobject.ObjectID.class
            };
    ...
    public static ObjectComposition getObjectComposition(int id) {
        ObjectComposition objectComposition = CupidBot.getClientThread().runOnClientThreadOptional(() -> CupidBot.getClient().getObjectDefinition(id))
                .orElse(null);
        if (objectComposition == null) return null;
        return objectComposition.getImpostorIds() == null ?
                objectComposition :
                CupidBot.getClientThread().runOnClientThreadOptional((objectComposition::getImpostor)).orElse(null);
    }
    ...
    public static boolean canWalkTo(TileObject tileObject, int distance) {
        ...
        if (tileObject instanceof GameObject) {
            ...
            if (CupidBot.getClient().isInInstancedRegion()) {
                var localPoint = LocalPoint.fromWorld(CupidBot.getClient(), worldPoint);
                worldPoint = WorldPoint.fromLocalInstance(CupidBot.getClient(), localPoint);
            }
    ...
    public static WorldArea getWorldArea(GameObject gameObject) {
        ...
        return new Rs2WorldArea(
                WorldPoint.fromLocal(CupidBot.getClient(), localSWTile),
                WorldPoint.fromLocal(CupidBot.getClient(), localNETile)
        );
    }
    ...
    public static boolean hoverOverObject(TileObject object) {
        ...
        CupidBot.getNaturalMouse().moveTo(point.getX(), point.getY());
        return true;
    }
}
