...
import net.runelite.client.plugins.cupidbot.configs.SpecialAttackConfigs;
import net.runelite.client.plugins.cupidbot.pouch.PouchScript;
...
import net.runelite.client.plugins.cupidbot.util.item.Rs2ItemManager;
...
import net.runelite.client.plugins.cupidbot.api.npc.Rs2NpcCache;
import net.runelite.client.plugins.cupidbot.api.player.Rs2PlayerCache;
import net.runelite.client.plugins.cupidbot.api.playerstate.Rs2PlayerStateCache;
import net.runelite.client.plugins.cupidbot.api.tileitem.Rs2TileItemCache;
import net.runelite.client.plugins.cupidbot.api.tileobject.Rs2TileObjectCache;
...
public class CupidBot {
    ...
    /**
     * PouchScript is injected in the main CupidBotPlugin as it's being used in multiple scripts
     */
    @Getter
    @Inject
    ...
    public static boolean isTelemetryDisabled() {
        if (Boolean.getBoolean("cupidbot.disableTelemetry")) {
            return true;
        }
        try {
            net.runelite.client.config.ConfigManager cm = configManager;
            if (cm == null) return false;
            String value = cm.getConfiguration(CupidBotConfig.configGroup, CupidBotConfig.keyDisableTelemetry);
    ...
    public static long getInstallSeed() {
        ...
        synchronized (CupidBot.class) {
            if (cachedInstallSeed != null) return cachedInstallSeed;
            long seed = 0L;
            try {
                net.runelite.client.config.ConfigManager cm = configManager;
                if (cm != null) {
                    String stored = cm.getConfiguration(CupidBotConfig.configGroup, INSTALL_SEED_KEY);
                    ...
                    if (seed == 0L) {
                        ...
                        if (seed == 0L) seed = 1L;
                        cm.setConfiguration(CupidBotConfig.configGroup, INSTALL_SEED_KEY, Long.toString(seed));
                    }
                }
    ...
    public static boolean hopToWorld(int worldNumber) {
        ...
        if (CupidBot.isHopping()) {
            log.error("Already hopping world");
            return true;
        }
        if (CupidBot.cantHopWorld) {
            log.error("Can't hop world, already trying to hop");
            return false;
        }
        boolean hopIssued = CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            ...
            if (quickHopTargetWorld != null || CupidBot.getClient().getGameState() != GameState.LOGGED_IN) {
                log.error("Quick hop target world is not null or game state is not logged in");
                return false;
            }
            if (CupidBot.getClient().getWorld() == worldNumber) {
                return true;
            }
            World newWorld = CupidBot.getWorldService().getWorlds().findWorld(worldNumber);
            if (newWorld == null) {
                CupidBot.getNotifier().notify("Invalid World");
            ...
            CupidBot.getClient().openWorldHopper();
            CupidBot.getClient().hopToWorld(rsWorld);
        ...
        if (CupidBot.getClient().getWorld() != worldNumber) {
            sleep(600);
            sleepUntil(() -> CupidBot.isHopping()
                            || CupidBot.getClient().getWorld() == worldNumber
                            || Rs2Widget.getWidget(193, 0) != null, 5000);
        }
        boolean hopping = CupidBot.isHopping() || CupidBot.getClient().getWorld() == worldNumber;
        if (!hopping) {
            ...
            if (confirmRoot != null) {
                ...
                if (switchWorldWidget != null && Rs2Widget.clickWidget(switchWorldWidget)) {
                    sleepUntil(() -> CupidBot.isHopping()
                                    || CupidBot.getClient().getWorld() == worldNumber, 4000);
                    hopping = CupidBot.isHopping() || CupidBot.getClient().getWorld() == worldNumber;
                }
            }
        }
        if (hopping) {
            // Block until the hop fully lands so callers don't race against HOPPING/LOGIN_SCREEN.
            sleepUntil(() -> CupidBot.getClient().getWorld() == worldNumber
                            && CupidBot.getClient().getGameState() == GameState.LOGGED_IN, 15000);
        }
        boolean success = CupidBot.getClient().getWorld() == worldNumber;
        if (!success) {
            log.error("Failed to hop to world {}", worldNumber);
        }
        return success;
    ...
    private static final int DEFAULT_MESSAGE_DISPOSE_MS = 10_000;

    public static void showMessage(String message) {
        showMessage(message, DEFAULT_MESSAGE_DISPOSE_MS);
    }
    ...
    public static void showMessage(String message, int disposeTime) {
        ...
        int disposeDelay = disposeTime > 0 ? disposeTime : DEFAULT_MESSAGE_DISPOSE_MS;
        Runnable messageRunnable = () ->
        {
            try {
                ...
                Timer timer = new Timer(disposeDelay, e -> dialog.dispose());
    ...
    public static List<Rs2ItemModel> updateItemContainer(int id, ItemContainerChanged e) {
        if (e.getContainerId() == id) {
            ...
            for (Item item : e.getItemContainer().getItems()) {
                ...
                ItemComposition composition = CupidBot.getItemManager().getItemComposition(item.getId());
     ...
     * <h3>Example:</h3>
     * <pre><code>
     * // show a popup titled "CupidBot" with a formatted enabled message
     * CupidBot.openPopUp(
     *     "CupidBot",
     *     String.format(
    ...
    public static boolean isPluginEnabled(Plugin plugin) {
        return plugin != null && CupidBot.getPluginManager().isPluginEnabled(plugin);
    }
    ...
    public static boolean shouldSkipVanillaClientDownload() {
        if (isDebug()) {
            try {
                ...
                if (CupidBot.isFirstRun()) {
                    CupidBot.writeVersionToFile(currentVersion);
                    System.out.println("First run in debug mode. Version written to file.");
                } else {
                    String storedVersion = CupidBot.readVersionFromFile();
    ...
    /**
     * Retrieves a list of active plugins that are part of the "cupidbot" package, excluding specific plugins.
     * <p>
     * This method filters the active plugins managed by the `pluginManager` to include only those whose
     * package name contains "cupidbot" (case-insensitive). It further excludes certain plugins based on
     * their class names, such as "QuestHelperPlugin", "MInventorySetupsPlugin", "CupidBotPlugin",
     * "ShortestPathPlugin", "AntibanPlugin", and "ExamplePlugin".
     *
     * @return a list of active plugins belonging to the "cupidbot" package, excluding the specified plugins.
     */
    public static List<Plugin> getActiveCupidBotPlugins() {
        return pluginManager.getActivePlugins().stream()
                .filter(x -> x.getClass().getPackage().getName().toLowerCase().contains("cupidbot"))
                .filter(x -> !x.getClass().getSimpleName().equalsIgnoreCase("QuestHelperPlugin")
                        ...
                        && !x.getClass().getSimpleName().equalsIgnoreCase("CupidBotPlugin")
    ...
    /**
     * Retrieves a list of active `Script` instances from the currently active cupidbot plugins.
     * <p>
     * This method iterates through all active cupidbot plugins and inspects their fields using reflection
     ...
     *
     * @return a list of active `Script` instances extracted from the cupidbot plugins.
     */
    public static List<Script> getActiveScripts() {
        return getActiveCupidBotPlugins().stream()
                .flatMap(x -> {
