...
This is the long-form guide for the CupidBot entity cache/query layer. Use singleton caches from `CupidBot.getRs2XxxCache()`; do not instantiate caches or queryables directly.
...
The **Queryable API** is a modern, fluent interface for querying game entities in CupidBot. It provides a type-safe, chainable way to filter and find NPCs, players, ground items, and tile objects with minimal boilerplate code.
...
```java
var npcCache = CupidBot.getRs2NpcCache();
var playerCache = CupidBot.getRs2PlayerCache();
var tileItemCache = CupidBot.getRs2TileItemCache();
var tileObjectCache = CupidBot.getRs2TileObjectCache();
...
Rs2NpcQueryable query = CupidBot.getRs2NpcCache().query()
        .withName("Guard")
        .within(10);
...
Always use the corresponding singleton cache from `CupidBot` and call `query()` from it to build queries against the per-tick cache.
...
```java
Rs2TileItemModel coins = CupidBot.getRs2TileItemCache().query()
        .withName("Coins")
        .nearestOnClientThread();
```
...
**Find nearest player:**
```java
Rs2PlayerModel player = CupidBot.getRs2PlayerCache().query()
        .withName("PlayerName")
        .nearestOnClientThread();
```
