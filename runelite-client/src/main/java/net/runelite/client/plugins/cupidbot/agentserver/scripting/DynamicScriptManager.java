package net.runelite.client.plugins.cupidbot.agentserver.scripting;
...
import net.runelite.client.plugins.cupidbot.externalplugins.PluginJarClassLoader;
...
public class DynamicScriptManager {
    ...
    public synchronized List<Map<String, Object>> listDeployments() {
        ...
        for (DeployedScript d : deployments.values()) {
            ...
            if (d.getPlugin() != null) {
                ...
                info.put("active", CupidBot.getPluginManager().isActive(d.getPlugin()));
            }
            result.add(info);
        }
        return result;
    ...
    private void doUndeploy(DeployedScript deployment) {
        if (deployment.getPlugin() != null) {
            ...
            CupidBot.getPluginManager().remove(deployment.getPlugin());
        }
    ...
    private Plugin instantiateAndStart(Class<?> clazz) throws PluginInstantiationException, DeploymentException {
        ...
        for (Plugin existing : CupidBot.getPluginManager().getPlugins()) {
        ...
        Injector parentInjector = CupidBot.getInjector();
        ...
        PluginManager pm = CupidBot.getPluginManager();
        pm.add(plugin);
