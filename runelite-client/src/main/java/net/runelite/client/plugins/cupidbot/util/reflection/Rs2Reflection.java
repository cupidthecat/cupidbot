...
public class Rs2Reflection {
    ...
    public static void invokeMenu(int param0, int param1, int opcode, int identifier, int itemId, int worldViewId, String option, String target, int canvasX, int canvasY)
    {
        ...
        if (menuAction == null)
        {
            MenuActionAsmResolver.Resolution resolution = MenuActionAsmResolver.resolve(CupidBot.getClient().getClass());
            if (resolution != null)
            {
        ...
        {
            CupidBot.showMessage("invokeMenu method is broken! Falling back to runelite menu action");
            CupidBot.getClientThread().invoke(() -> CupidBot.getClient().menuAction(param0, param1, MenuAction.of(opcode), identifier, itemId, option, target));
            return;
        }

        menuAction.setAccessible(true);
        CupidBot.getClientThread().runOnClientThreadOptional(() -> menuAction.invoke(null, param0, param1, opcode, identifier, itemId, worldViewId, option, target, canvasX, canvasY, menuActionGarbageValue));
        menuAction.setAccessible(false);

        if (CupidBot.getClient().getKeyboardIdleTicks() > Rs2Random.between(5000, 10000))
        {
