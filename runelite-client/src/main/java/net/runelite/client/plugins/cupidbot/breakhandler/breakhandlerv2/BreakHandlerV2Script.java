...
import net.runelite.client.plugins.cupidbot.util.discord.Rs2Discord;
...
import net.runelite.client.plugins.cupidbot.util.world.Rs2WorldUtil;
...
public class BreakHandlerV2Script extends Script {
    ...
    private void loadActiveProfile() {
        if (config.useActiveProfile()) {
            try {
                activeProfile = CupidBot.getConfigManager().getProfile();
    ...
    private void handleBreakRequested() {
        ...
        if (CupidBot.isLoggedIn()) {
            preBreakWorld = CupidBot.getClient().getWorld();
        }
    ...
    private void handleLoggingIn() {
        ...
        if (CupidBot.isLoggedIn()) {
            ...
            sendDiscordNotification("Login Successful",
                "Logged into world " + CupidBot.getClient().getWorld());
    ...
    private void detectUnexpectedLogout() {
        ...
        GameState gameState = CupidBot.getClient() != null ? CupidBot.getClient().getGameState() : null;
    ...
    private void enforceLogoutDuringActiveBreak() {
        ...
        if (breakRemainingSeconds <= 0 || !CupidBot.isLoggedIn() || !logoutBreakActive) {
            return;
        }
	...
	private int selectWorld() {
		...
		if (config.ignoreWorldSwitching()) {
			int currentWorld = CupidBot.getClient() != null ? CupidBot.getClient().getWorld() : -1;
    ...
    /**
     * Stops a configured CupidBot plugin once per break cycle.
     */
    private void stopConfiguredPluginIfNeeded() {
        ...
        String normalizedSelection = PluginStopHelper.normalizeStoredValue(config.pluginToStop(), CupidBot.getPluginManager());
        ...
        boolean wasEnabled = CupidBot.isPluginEnabled(pluginInstance);
        boolean stopResult = CupidBot.stopPlugin(pluginInstance);
        boolean nowEnabled = CupidBot.isPluginEnabled(pluginInstance);

        log.info("[BreakHandlerV2] Stop request for {} -> {} (wasEnabled={}, nowEnabled={}, stopResult={})",
                PluginStopHelper.resolveDisplayName(normalizedSelection, CupidBot.getPluginManager()),
    ...
    private void startConfiguredPluginIfNeeded() {
        ...
        boolean started = pluginInstance != null
                ? CupidBot.startPlugin(pluginInstance)
                : CupidBot.startPlugin(stoppedPluginClassName);
        boolean nowEnabled = CupidBot.isPluginEnabled(pluginInstance != null ? pluginInstance : CupidBot.getPlugin(stoppedPluginClassName));

        log.info("[BreakHandlerV2] Restart request for {} -> {} (startedCall={}, nowEnabled={})",
                PluginStopHelper.resolveDisplayName(stoppedPluginClassName, CupidBot.getPluginManager()),
    ...
    private void attemptPluginRestartIfLoggedIn() {
        if (!pluginRestartPending || !CupidBot.isLoggedIn()) {
            return;
        }
    ...
    private void transitionToState(BreakHandlerV2State newState) {
        ...
        if (newState == BreakHandlerV2State.LOGGED_OUT) {
            net.runelite.client.plugins.cupidbot.util.antiban.SessionFatigue.endSession();
        }
    }
    ...
    private void restorePersistedBreakState() {
        ...
        Long savedEnd = CupidBot.getConfigManager().getConfiguration(BreakHandlerV2Config.configGroup, PERSISTED_BREAK_END_KEY, Long.class);
        Boolean savedLogout = CupidBot.getConfigManager().getConfiguration(BreakHandlerV2Config.configGroup, PERSISTED_BREAK_LOGOUT_KEY, Boolean.class);
    ...
    private void persistBreakState(boolean logoutBreak) {
        if (CupidBot.getConfigManager() == null || breakEndTime == null) {
            return;
        }
        logoutBreakActive = logoutBreak;
        CupidBot.getConfigManager().setConfiguration(BreakHandlerV2Config.configGroup, PERSISTED_BREAK_END_KEY, breakEndTime.toEpochMilli());
        CupidBot.getConfigManager().setConfiguration(BreakHandlerV2Config.configGroup, PERSISTED_BREAK_LOGOUT_KEY, logoutBreak);
    }
    ...
    private void clearPersistedBreakState() {
        ...
        logoutBreakActive = false;
        CupidBot.getConfigManager().unsetConfiguration(BreakHandlerV2Config.configGroup, PERSISTED_BREAK_END_KEY);
        CupidBot.getConfigManager().unsetConfiguration(BreakHandlerV2Config.configGroup, PERSISTED_BREAK_LOGOUT_KEY);
    }
