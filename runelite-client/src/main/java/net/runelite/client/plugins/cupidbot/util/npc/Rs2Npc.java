package net.runelite.client.plugins.cupidbot.util.npc;
...
public class Rs2Npc {
    ...
    public static Stream<Rs2NpcModel> getNpcs(Predicate<Rs2NpcModel> predicate) {
        try {
            ...
            Rs2NpcModel[] npcArray = CupidBot.getClientThread().runOnClientThreadOptional(() -> {
                ...
                if (CupidBot.getClient().getTopLevelWorldView() == null) {
                    log.warn("TopLevelWorldView is null, returning empty NPC stream");
                    return EMPTY_ARRAY;
                }

                if (CupidBot.getClient().getTopLevelWorldView().npcs() == null) {
                ...
                if (CupidBot.getClient().getLocalPlayer().getLocalLocation() == null) {
                ...
                final Stream<? extends NPC> npcStream = CupidBot.getClient().getTopLevelWorldView().npcs().stream();
                final LocalPoint playerLocation = CupidBot.getClient().getLocalPlayer().getLocalLocation();
    ...
    public static Stream<Rs2NpcModel> getAttackableNpcs(boolean reachable) {
        ...
        Stream<Rs2NpcModel> npcs = getNpcs(npc -> npc.getCombatLevel() > 0
                && !npc.isDead()
                && (!npc.isInteracting() || Objects.equals(npc.getInteracting(), CupidBot.getClient().getLocalPlayer())));
    ...
    public static Rs2NpcModel[] getPestControlPortals() {
        return getNpcs("portal", false)
                ...
                .sorted(Comparator.comparingInt(value -> value.getLocalLocation().distanceTo(CupidBot.getClient().getLocalPlayer().getLocalLocation())))
                .toArray(Rs2NpcModel[]::new);
    }
    ...
    public static Optional<Rs2NpcModel> getNpc(int id, List<Integer> excludedIndexes) {
        return getNpcs(x -> x.getId() == id && !excludedIndexes.contains(x.getIndex()))
                .min(Comparator.comparingInt(value -> value.getLocalLocation().distanceTo(CupidBot.getClient().getLocalPlayer().getLocalLocation())));
    }
    ...
    public static Rs2NpcModel getRandomEventNPC() {
        return getNpcs(npc -> {
            ...
            return npcActions.contains("Dismiss") && Objects.equals(npc.getInteracting(), CupidBot.getClient().getLocalPlayer());
    ...
    public static boolean hasAction(int id, String action) {
        NPCComposition npcComposition = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                CupidBot.getClient().getNpcDefinition(id)).orElse(null);
    ...
    public static boolean interact(Rs2NpcModel npc, String action) {
        ...
        CupidBot.status = action + " " + npc.getName();
        try {
            ...
            final NPCComposition npcComposition = CupidBot.getClientThread().runOnClientThreadOptional(
                    () -> CupidBot.getClient().getNpcDefinition(npc.getId())).orElse(null);
    ...
    public static boolean attack(NPC npc) {
        ...
        if (npc.isInteracting() && !Objects.equals(npc.getInteracting(), CupidBot.getClient().getLocalPlayer()) && !Rs2Player.isInMulti())
            return false;
    ...
    public static boolean canWalkTo(Rs2NpcModel npc, int distance) {
        ...
        var localLocation = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), location);
    ...
    public static Rs2NpcModel getNearestNpcWithAction(String action) {
        Rs2WorldPoint playerLocation = new Rs2WorldPoint(Rs2Player.getWorldLocation());
        boolean isInstance = CupidBot.getClient().getTopLevelWorldView().getScene().isInstance();
        return getNpcs()
    ...
    public static String getAvailableAction(Rs2NpcModel npc, List<String> possibleActions) {
        ...
        NPCComposition composition = CupidBot.getClientThread().runOnClientThreadOptional(
                        () -> CupidBot.getClient().getNpcDefinition(npc.getId()))
                .orElse(null);
    ...
    public static boolean isInAttackRange(Rs2NpcModel npc, boolean includeManualCast, boolean includeSpecialAttack) {
        ...
        LocalPoint playerLocal = CupidBot.getClient().getLocalPlayer().getLocalLocation();
