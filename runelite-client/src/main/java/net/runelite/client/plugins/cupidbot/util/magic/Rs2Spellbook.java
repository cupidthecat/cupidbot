...
public enum Rs2Spellbook {
    ...
    public static Rs2Spellbook getCurrentSpellbook() {
        return fromValue(CupidBot.getVarbitValue(VarbitID.SPELLBOOK));
    }
    ...
    public static boolean isSpellbookChangedByLunar() {
        return CupidBot.getVarbitValue(VarbitID.LUNAR_SPELLBOOK_CHANGE) == 1;
    }
    ...
    public boolean switchTo() {
        if (!isUnlocked()) {
            CupidBot.log("Cannot switch to " + this.name() + " spellbook - not unlocked");
            return false;
        }

        if (getCurrentSpellbook() == this) {
            CupidBot.log("Already on " + this.name() + " spellbook");
            return true;
        }
        ...
        switch (this) {
            ...
            case LUNAR:
                ...
                if (switchLocation == null) {
                    CupidBot.log("No switch location defined for " + this.name() + " spellbook");
                    return false;
                }
                return switchViaAltar();
            ...
            default:
                CupidBot.log("Unknown switch method for " + this.name() + " spellbook");
                return false;
        }
    }
    ...
    private boolean switchViaAltar() {
            ...
            switch (currentSpellbook) {
                ...
                case ARCEUUS:
                    ...
                    if (!Rs2GameObject.getGameObjects(o -> o.getId() == ANCIENT.getGameID(), maxSearchRadius).isEmpty()) {
                        ...
                        CupidBot.log("No suitable altar found to switch from Arceuus to Modern spellbook");
                        return false;
                    }
                    break;
                default:
                    CupidBot.log("Already on Modern spellbook");
                    return true;
            }
        }

        if (targetAltarSpellbook.getGameID() == null || targetAltarSpellbook.getGameID() == -1) {
            CupidBot.log("TODO: Find altar object ID for " + targetAltarSpellbook.name() + " spellbook");
            return false;
        }

        if (targetAltarSpellbook.getSwitchLocation() == null || currentPlayerLocation == null) {
            CupidBot.log("No switch location defined for " + targetAltarSpellbook.name() + " spellbook");
            return false;
        }
        ...
        if (currentPlayerLocation.distanceTo(targetAltarSpellbook.getSwitchLocation()) >= maxSearchRadius) {
            CupidBot.log("Player is not at the correct location for " + targetAltarSpellbook.name() +
        ...
        if (altarObject == null) {
            CupidBot.log("No altar object found for " + targetAltarSpellbook.name() +
    ...
    public static boolean switchViaNpc() {
        switch (getCurrentSpellbook()) {
            ...
            default:
                CupidBot.log("Tyss NPC switching not supported for current spellbook: " + getCurrentSpellbook());
                return false;
        }
    ...
    public static WorldPoint getNearestSwitchLocation(boolean includeBankeItems) {
        ...
        if (locationIndex == -1) {
            CupidBot.log("No spellbook switch locations found nearby");
