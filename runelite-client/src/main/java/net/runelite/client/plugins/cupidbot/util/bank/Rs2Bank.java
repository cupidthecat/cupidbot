...
import net.runelite.client.plugins.cupidbot.util.inventory.RunePouchType;
...
import net.runelite.client.plugins.cupidbot.util.misc.Predicates;
...
import static net.runelite.client.plugins.cupidbot.util.gameobject.Rs2GameObject.hoverOverObject;
import static net.runelite.client.plugins.cupidbot.util.npc.Rs2Npc.hoverOverActor;
...
public class Rs2Bank {
    ...
    private static final String CONFIG_GROUP = "cupidbot";
    ...
    private static ItemComposition getItemDefinitionThreadSafe(int id)
    {
        ...
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> c.getItemDefinition(id)).orElse(null);
    }
	...
	public static void updateLocalBank(ItemContainerChanged event) {
		assert CupidBot.getClient().isClientThread();
		...
		for (int slot = 0; slot < items.length; slot++) {
			...
			final ItemComposition itemComposition = CupidBot.getClient().getItemDefinition(item.getId());
    ...
    public static boolean depositEquipment() {
        ...
        CupidBot.getMouse().click(widget.getBounds());
        return true;
    }
    ...
    private static boolean depositOne(Rs2ItemModel rs2Item) {
        ...
        if (CupidBot.getVarbitValue(VarbitID.BANK_QUANTITY_TYPE) == 0) {
    ...
    private static boolean handleAmount(Rs2ItemModel rs2Item, int amount, boolean safe) {
        ...
        int selected = CupidBot.getVarbitValue(VarbitID.BANK_QUANTITY_TYPE);
        int configuredX = CupidBot.getVarbitValue(VarbitID.BANK_REQUESTEDQUANTITY);
    ...
    private static boolean depositAll(Rs2ItemModel rs2Item) {
        ...
        if (CupidBot.getVarbitValue(VarbitID.BANK_QUANTITY_TYPE) == 4) {
            invokeMenu(2, rs2Item);
        } else {
    ...
    public static boolean emptyContainers() {
        CupidBot.status = "Empty containers";
    ...
    private static boolean withdrawOne(Rs2ItemModel rs2Item) {
        ...
        final int entryIndex = CupidBot.getVarbitValue(VarbitID.BANK_QUANTITY_TYPE) == 0 ? 1 : 2;
    ...
    public static boolean openBank() {
        CupidBot.status = "Opening bank";

        try {
            if (CupidBot.getClient().isWidgetSelected()) {
                CupidBot.getMouse().click();
            }
            ...
            CupidBot.logStackTrace("Rs2Bank", ex);
            return false;
        }
    }
    ...
    public static boolean openCollectionBox() {
        CupidBot.status = "Opening collection box";
    ...
    public static boolean openBank(Rs2NpcModel npc) {
        ...
        try {
            if (isOpen()) return true;
            if (Rs2Inventory.isItemSelected()) CupidBot.getMouse().click();
    ...
    private static AbstractMap.SimpleEntry<List<WorldPoint>, BankLocation> getPathAndBankToNearestBank(WorldPoint worldPoint, int maxObjectSearchRadius) {
        CupidBot.log("Finding nearest bank...");
        ...
        if (Objects.equals(Rs2Player.getWorldLocation(), worldPoint)) {
            ...
            if (byObject.isPresent() && byObject.get().hasRequirements()) {
                CupidBot.log("Found nearest bank (object): " + byObject.get());
        ...
        if (accessibleBanks.isEmpty()) {
            CupidBot.log("No accessible banks found");
            return null;
        }
        ...
        if (path.isEmpty()) {
            CupidBot.log("Unable to find path to nearest bank");
            return null;
        }
        ...
        if (byPath.isPresent()) {
            CupidBot.log("Found nearest bank (shortest path): " + byPath.get());
            returnBankLocation = byPath.get();
        } else {
            CupidBot.log("Nearest bank point " + nearestTile + " did not match any BankLocation");
        }
        return new AbstractMap.SimpleEntry<>(path, returnBankLocation);
    }
    ...
    public static boolean walkToBank(BankLocation bankLocation, boolean toggleRun) {
        ...
        CupidBot.status = "Walking to nearest bank " + bankLocation.toString();
    ...
    public static boolean walkToBankAndUseBank(BankLocation bankLocation, boolean toggleRun) {
        ...
        boolean result = Rs2Walker.getDistanceBetween(playerLocation, bankLocation.getWorldPoint()) <= 8;
        if (!result) {
    ...
    public static boolean isWithdrawAs(boolean noted) {
        final boolean isNoted = CupidBot.getVarbitValue(VarbitID.BANK_WITHDRAWNOTES) == 1;
    ...
    public static boolean withdrawLootItems(String npcName, List<String> itemsToNotSell) {
        ...
        for (LootTrackerRecord lootTrackerRecord : CupidBot.getAggregateLootRecords()) {
            ...
            for (LootTrackerItem lootTrackerItem : lootTrackerRecord.getItems()) {
                ...
                ItemComposition itemComposition = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                        CupidBot.getClient().getItemDefinition(lootTrackerItem.getId())).orElse(null);
                ...
                if (isNoted) {
                    ...
                    itemComposition = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                            CupidBot.getClient().getItemDefinition(unnotedItemId)).orElse(null);
    ...
    public static List<Widget> getTabs() {
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            Widget bankContainerWidget = CupidBot.getClient().getWidget(ComponentID.BANK_TAB_CONTAINER);
    ...
    public static List<Widget> getItems() {
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            Widget bankContainerWidget = CupidBot.getClient().getWidget(BANK_ITEM_CONTAINER);
            if (bankContainerWidget != null) {
                // Get children and filter out the tabs that don't have the Action Collapse tab
                return Arrays.asList(bankContainerWidget.getDynamicChildren());
            }
    ...
    public static int getCurrentTab() {
        return CupidBot.getVarbitValue(VarbitID.BANK_CURRENTTAB);
    }
    ...
    private static void updateTabCounts() {
        bankTabCounts[0] = CupidBot.getVarbitValue(VarbitID.BANK_TAB_1);
        bankTabCounts[1] = CupidBot.getVarbitValue(VarbitID.BANK_TAB_2);
        bankTabCounts[2] = CupidBot.getVarbitValue(VarbitID.BANK_TAB_3);
        bankTabCounts[3] = CupidBot.getVarbitValue(VarbitID.BANK_TAB_4);
        bankTabCounts[4] = CupidBot.getVarbitValue(VarbitID.BANK_TAB_5);
        bankTabCounts[5] = CupidBot.getVarbitValue(VarbitID.BANK_TAB_6);
        bankTabCounts[6] = CupidBot.getVarbitValue(VarbitID.BANK_TAB_7);
        bankTabCounts[7] = CupidBot.getVarbitValue(VarbitID.BANK_TAB_8);
        bankTabCounts[8] = CupidBot.getVarbitValue(VarbitID.BANK_TAB_9);
    }
    ...
    private static int calculateScrollYFromSlotId(int slotId) {
        ...
        Widget w = CupidBot.getClient().getWidget(BANK_ITEM_CONTAINER);
    ...
    public static boolean scrollBankToSlot(int slotId) {
        ...
        CupidBot.getClientThread().invokeLater(() -> {
            CupidBot.getClient().setVarcIntValue(VarClientInt.BANK_SCROLL, scrollY);
            CupidBot.getClient().runScript(ScriptID.UPDATE_SCROLLBAR, ComponentID.BANK_SCROLLBAR, BANK_ITEM_CONTAINER, scrollY);
        });
    ...
    public static boolean preHover() {
        if (!Rs2AntibanSettings.naturalMouse) {
            if(Rs2AntibanSettings.devDebug)
                CupidBot.log("Natural mouse is not enabled, can't hover");
            return false;
        }
        ...
        CupidBot.status = "Hovering over bank";
        ...
        try {
            ...
            CupidBot.log("No bank objects or NPC found to hover over.");
        } catch (Exception ex) {
            CupidBot.log("An error occurred while hovering over the bank: " + ex.getMessage());
        }
    ...
    public static boolean isLockedSlot(int slot) {
        ...
        Widget container = CupidBot.getClient().getWidget(BANK_INVENTORY_ITEM_CONTAINER);
    ...
    private static boolean toggleItemLock(Rs2ItemModel rs2Item)
    {
        ...
        container = BANK_INVENTORY_ITEM_CONTAINER;
        final int currentLockState = CupidBot.getVarbitValue(VarbitID.BANK_SIDE_SLOT_OVERVIEW);
        invokeMenu(10, rs2Item);
        return sleepUntilTrue(() -> CupidBot.getVarbitValue(VarbitID.BANK_SIDE_SLOT_OVERVIEW) != currentLockState, 300, 2000);
    }
