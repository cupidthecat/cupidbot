package net.runelite.client.plugins.cupidbot.util.magic;
...
import net.runelite.client.plugins.cupidbot.util.magic.thralls.Rs2Thrall;
...
import net.runelite.client.plugins.cupidbot.util.settings.Rs2SpellBookSettings;
...
public class Rs2Magic {
    ...
    public static boolean canCast(MagicAction magicSpell) {
        if (!oneTimeSpellBookCheck()) {
            ...
            CupidBot.log("Your spellbook filtering seems off...CupidBot is trying to fix this");
            return false;
        }
        ...
        if (!isSpellbook(magicSpell.getSpellbook())) {
            CupidBot.log("You need to be on the " + magicSpell.getSpellbook() + " spellbook to cast " + magicSpell.getName() + ".");
            return false;
        }
    ...
    public static boolean cast(MagicAction magicSpell, String option, int identifier) {
        ...
        Rs2Tab.switchToMagicTab();
        CupidBot.status = "Casting " + magicSpell.getName();
        sleep(150, 300);
    ...
    public static boolean quickCast(MagicAction magicSpell) {
        ...
        if(quickCanCast(magicSpell)) {
            ...
            CupidBot.click(widget.getBounds());
            return true;
        }
        log("Unable to cast " + magicSpell.getName());
        return false;
    ...
    public static boolean castOn(MagicAction magicSpell, Actor actor) {
        ...
        cast(magicSpell);
        if (!Global.sleepUntil(() -> CupidBot.getClient().isWidgetSelected())) return false;
    ...
    public static void alch(Rs2ItemModel item, int sleepMin, int sleepMax) {
        if (CupidBot.getClient().getRealSkillLevel(Skill.MAGIC) >= 55) {
    ...
    private static void interact(Rs2ItemModel item, Point point, String info) {
        if (item == null) {
            CupidBot.status = info + " x: " + point.getX() + " y: " + point.getY();
            CupidBot.getMouse().click(point);
        } else {
            CupidBot.status = info + " " + item.getName();
    ...
    private static void alch(MagicAction magicSpell, Rs2ItemModel item, int sleepMin, int sleepMax) {
        ...
        sleepUntil(() -> CupidBot.getClientThread().runOnClientThreadOptional(() -> Rs2Tab.getCurrentTab() == InterfaceTab.MAGIC).orElse(false), 5000);
        sleep(sleepMin, sleepMax);
        if (!cast(magicSpell)) return;
        sleepUntil(() -> CupidBot.getClientThread().runOnClientThreadOptional(() -> Rs2Tab.getCurrentTab() == InterfaceTab.INVENTORY).orElse(false), 5000);
        sleep(sleepMin, sleepMax);
    ...
    public static boolean npcContact(String npcName) {
        if (!isSpellbook(Rs2Spellbook.LUNAR)) {
            CupidBot.log("Tried casting npcContact, but lunar spellbook was not found.");
            return false;
        }
        ...
        if (!Rs2UiHelper.isRectangleWithinRectangle(chooseCharacterBounds, npcBounds)) {
            CupidBot.log("NPC widget is not fully visible inside the choose character widget, scrolling...");
            Global.sleepUntil(() -> Rs2UiHelper.isRectangleWithinRectangle(chooseCharacterBounds, Rs2Widget.findWidget(npcName).getBounds()), () -> {
                ...
                if (isBelow) CupidBot.getMouse().scrollDown(Rs2UiHelper.getClickingPoint(chooseCharacterWidget.getBounds(),true));
                else CupidBot.getMouse().scrollUp(Rs2UiHelper.getClickingPoint(chooseCharacterWidget.getBounds(),true));
            }, 5000, 300);
        }
    ...
    public static boolean isShadowVeilActive() {
        return CupidBot.getVarbitValue(VarbitID.ARCEUUS_SHADOW_VEIL_ACTIVE) == 1;
    }
    ...
    public static Rs2CombatSpells getCurrentAutoCastSpell() {
        final int autoCastSpell = CupidBot.getVarbitValue(VarbitID.AUTOCAST_SPELL) - (isSpellbook(Rs2Spellbook.ANCIENT) ? ANCIENT_VARBIT_OFFSET : 0);
