...
public class GameChatAppender extends AppenderBase<ILoggingEvent> {
    ...
    private static volatile boolean onlyCupidBotLogging = true;
    ...
    public GameChatAppender(String pattern) {
        ...
        addFilter(new OnlyCupidBotLoggingFilter());
     ...
     */
    public static void updateConfiguration(boolean enabled, Level level, boolean cupidbotOnly) {
        loggingEnabled = enabled;
        minimumLevel = level;
        onlyCupidBotLogging = cupidbotOnly;
    }
    ...
    protected void append(ILoggingEvent event) {
        ...
        CupidBot.getClientThread().invoke(() ->
                CupidBot.getClient().addChatMessage(ChatMessageType.ENGINE, "", formatted, "", false)
        );
    }
    ...
    private static class OnlyCupidBotLoggingFilter extends Filter<ILoggingEvent> {
        @Override
        public FilterReply decide(ILoggingEvent event) {
            // If only cupidbot logging is disabled, accept all logs
            if (!onlyCupidBotLogging) {
                return FilterReply.NEUTRAL;
            }
            
            // Otherwise, only accept cupidbot logs
            return event.getLoggerName().startsWith("net.runelite.client.plugins.cupidbot") ? FilterReply.NEUTRAL : FilterReply.DENY;
        }
    }
}
