...
public class ScriptHandler extends AgentHandler {
	...
	private void handleList(HttpExchange exchange) throws IOException {
		...
		for (Plugin plugin : pm.getPlugins()) {
			if (!isCupidBotPlugin(plugin)) continue;
	...
	private PluginManager getPluginManager() {
		try {
			return CupidBot.getPluginManager();
		} catch (Exception e) {
			return null;
		}
	}
	...
	private Plugin resolvePlugin(Map<String, Object> body) {
		...
		String className = (String) body.get("className");
		if (className != null && !className.isEmpty()) {
			return CupidBot.getPlugin(className);
		}
	...
	private boolean isCupidBotPlugin(Plugin plugin) {
		return plugin.getClass().getPackageName().toLowerCase().contains("cupidbot");
	}
}
