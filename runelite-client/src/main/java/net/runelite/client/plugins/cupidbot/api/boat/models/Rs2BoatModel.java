package net.runelite.client.plugins.cupidbot.api.boat.models;
...
import net.runelite.client.plugins.cupidbot.api.boat.data.BoatType;
import net.runelite.client.plugins.cupidbot.api.boat.data.Heading;
import net.runelite.client.plugins.cupidbot.api.boat.data.PortTaskData;
import net.runelite.client.plugins.cupidbot.api.boat.data.PortTaskVarbits;
...
public class Rs2BoatModel implements WorldEntity, IEntity {
    ...
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(boat::getWorldView).orElse(null);
    }

    @Override
    public LocalPoint getCameraFocus()
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(boat::getCameraFocus).orElse(null);
    }

    @Override
    public LocalPoint getLocalLocation()
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(boat::getLocalLocation).orElse(null);
    }
    ...
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(boat::getOrientation).orElse(0);
    }

    @Override
    public LocalPoint getTargetLocation()
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(boat::getTargetLocation).orElse(null);
    }

    @Override
    public int getTargetOrientation()
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(boat::getTargetOrientation).orElse(0);
    }

    @Override
    public LocalPoint transformToMainWorld(LocalPoint point)
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> boat.transformToMainWorld(point)).orElse(null);
    }

    @Override
    public boolean isHiddenForOverlap()
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(boat::isHiddenForOverlap).orElse(false);
    }

    @Override
    public WorldEntityConfig getConfig()
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(boat::getConfig).orElse(null);
    }

    @Override
    public int getOwnerType()
    {
        return CupidBot.getClientThread().runOnClientThreadOptional(boat::getOwnerType).orElse(OWNER_TYPE_NOT_PLAYER);
    }
    ...
    {
        int boatTypeValue = CupidBot.getVarbitPlayerValue(VarPlayerID.SAILING_SIDEPANEL_BOAT_TYPE);
    ...
    {
        return CupidBot.getVarbitValue(VarbitID.SAILING_SIDEPANEL_PLAYER_AT_HELM) == 1;
    }
    ...
    public boolean navigate()
    {
        ...
        CupidBot.getRs2TileObjectCache().query().withId(getSteeringForBoatType()).interact("Navigate");
    ...
    public WorldPoint getPlayerBoatLocation()
    {
        ...
        return CupidBot.getClientThread().invoke(() ->
        {
            ...
            return CupidBot.getClientThread().invoke(() -> WorldPoint.fromLocal(
    ...
    public boolean boardBoat()
    {
        ...
        CupidBot.getRs2TileObjectCache().query().withIds(GANGPLANK_IDS).interact("Board");
    ...
    public boolean disembarkBoat()
    {
        ...
        CupidBot.getRs2TileObjectCache().query().withIds(GANGPLANK_IDS).interact("Disembark");
        sleepUntil(() -> !isOnBoat(), 5000);
        return !isOnBoat();
    }
    ...
    {
        return CupidBot.getVarbitValue(VarbitID.SAILING_SIDEPANEL_BOAT_MOVE_MODE);
    }
    ...
    public void setHeading(Heading heading)
    {
        ...
        if (worldview == null)
        {
            menuEntry.setWorldViewId(CupidBot.getClient().getTopLevelWorldView().getId());
        }
        else
        {
            menuEntry.setWorldViewId(worldview.getId());
        }
        CupidBot.doInvoke(menuEntry, new java.awt.Rectangle(1, 1, CupidBot.getClient().getCanvasWidth(), CupidBot.getClient().getCanvasHeight()));
    }
    ...
    public boolean trimSails()
    {
        ...
        CupidBot.getRs2TileObjectCache().query().fromWorldView().withIds(SAIL_IDS).interact("trim");
        return sleepUntil(() -> CupidBot.isGainingExp, 5000);
    }
    ...
    public boolean openCargo()
    {
        ...
        return CupidBot.getRs2TileObjectCache().query().withIds(CARGO_HOLD_IDS).interact("open");
    }

    public Map<PortTaskVarbits, Integer> getPortTasksVarbits()
    {
        return Arrays.stream(PortTaskVarbits.values())
                .map(x -> Map.entry(x, CupidBot.getVarbitValue(x.getId())))
