...
public class CupidBotMouseOverlay extends Overlay {
    private final Client client;
    private final DevToolsPlugin plugin;
    private float angle = 0.0f; // Rotation angle

    @Inject
    CupidBotMouseOverlay(Client client, DevToolsPlugin plugin) {
        this.client = client;
        this.plugin = plugin;
    ...
    public Dimension render(Graphics2D g) {
        if (plugin.getMouseMovement().isActive()) {
            if (!CupidBot.getMouse().getTimer().isRunning()) {
                CupidBot.getMouse().getPoints().clear();
                CupidBot.getMouse().getTimer().start();
            }
            ...
            // Rotate the graphics
            g2d.setColor(CupidBot.getMouse().getRainbowColor());
            ...
            g2d.dispose();
            // Mouse position
            int x = CupidBot.getMouse().getLastMove().getX();
            int y = CupidBot.getMouse().getLastMove().getY();
            ...
            //OverlayUtil.renderTextLocation(g, new net.runelite.api.Point(drawX, drawY), "✛", CupidBot.getMouse().getRainbowColor());
            ...
            //g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            var points = CupidBot.getMouse().getPoints();
			...
			if (pointArray.length > 1)
			{
				...
				if (firstPoint != null && lastPoint != null)
				{
					...
					g.setColor(CupidBot.getMouse().getRainbowColor());
