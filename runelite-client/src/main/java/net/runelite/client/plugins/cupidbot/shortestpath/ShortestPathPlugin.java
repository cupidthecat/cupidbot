 ...
 *
 * Any modifications to the original code have been made by CupidBot.
 */
...
import net.runelite.client.plugins.cupidbot.shortestpath.pathfinder.SplitFlagMap;
...
@PluginDescriptor(
        ...
        tags = {"pathfinder", "map", "waypoint", "navigation", "cupidbot"},
...
public class ShortestPathPlugin extends Plugin implements KeyListener {
        ...
        if (targets == null || targets.isEmpty()) {
            ...
            if (startPointSet && pathfinder != null) {
                ...
                if (exitPortal != null && inInstance) {
                    CupidBot.log("[ShortestPath] In POH instance — remapping pathfinder start "
    ...
    private void toggleCategory(String categoryName, Function<ShortestPathPanel, WorldPoint> targetFn) {
        ...
        if (p == null || !CupidBot.isLoggedIn()) {
            return;
        }
        WorldPoint target = targetFn.apply(p);
        if (target == null) {
            CupidBot.log("WebWalker: no " + categoryName + " selected in the panel.");
            return;
        }
    ...
    public void keyPressed(KeyEvent e) {
        if (client == null || !CupidBot.isLoggedIn())
        {
            return;
        }
        /**
    ...
    private final HotkeyListener nearestBankHotkeyListener = new HotkeyListener(() -> config.nearestBankHotkey()) {
        @Override
        public void hotkeyPressed() {
            if (panel == null || !CupidBot.isLoggedIn()) return;
