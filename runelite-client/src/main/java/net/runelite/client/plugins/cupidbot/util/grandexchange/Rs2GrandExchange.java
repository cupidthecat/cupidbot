package net.runelite.client.plugins.cupidbot.util.grandexchange;
...
import net.runelite.client.plugins.cupidbot.util.grandexchange.models.*;
...
public class Rs2GrandExchange {
    ...
    public static void closeExchange() {
        CupidBot.status = "Closing Grand Exchange";
        if (!isOpen()) {
            return;
        }
    ...
    public static void backToOverview() {
        CupidBot.status = "Back to overview";
    ...
    public static boolean openExchange() {
        CupidBot.status = "Opening Grand Exchange";
        try {
    ...
    private static void viewOffer(Widget widget) {
        ...
        CupidBot.doInvoke(menuEntry, bounds);
    }
    ...
    private static void adjustPriceByPercent(int percent) {
        ...
        int basePrice = CupidBot.getVarbitValue(VarbitID.GE_NEWOFFER_TYPE);
        ...
        if (absPercent % 5 == 0) {
            ...
            if (adjust5Widget == null) {
                CupidBot.log("Unable to find +-5% button widget.");
                return;
            }
            ...
            if (adjustXWidget == null) {
                CupidBot.log("Unable to find +-X% button widget.");
                return;
            }
...
//					MenuEntryImpl(getOption=Customise, getTarget=, getIdentifier=2, getType=CC_OP, getParam0=15, getParam1=30474266, getItemId=-1, isForceLeftClick=false, getWorldViewId=-1, isDeprioritized=false)
    ...
    private static boolean setQuantity(int quantity) {
        ...
        while (quantity != getOfferQuantity()) {
            ...
            CupidBot.getMouse().click(quantityButtonX.getBounds());
    ...
    private static void setPrice(int price) {
        if (price != getOfferPrice()) {
            ...
            CupidBot.getMouse().click(pricePerItemButtonX.getBounds());
    ...
    public static boolean collectAll(boolean collectToBank) {
        ...
        CupidBot.doInvoke(entry, bounds);
        return true;
    }
    ...
    public static Pair<Widget, Integer> getSearchResultWidget(String search, boolean exact) {
        Widget parent = CupidBot.getClientThread().runOnClientThreadOptional(() -> CupidBot.getClient().getWidget(InterfaceID.Chatbox.MES_LAYER_SCROLLCONTENTS)).orElse(null);
    ...
    public static boolean hasBoughtOffer() {
        return Arrays.stream(CupidBot.getClient().getGrandExchangeOffers()).anyMatch(x -> x.getState() == GrandExchangeOfferState.BOUGHT);
    }
    ...
    public static boolean hasSoldOffer() {
        return Arrays.stream(CupidBot.getClient().getGrandExchangeOffers()).anyMatch(x -> x.getState() == GrandExchangeOfferState.SOLD);
    }
    ...
    public static boolean hasBoughtOffer(GrandExchangeSlots slot) {
        ...
        GrandExchangeOffer offer = CupidBot.getClient().getGrandExchangeOffers()[slot.ordinal()];
    ...
    public static GrandExchangeSlots findSlotForItem(String itemName, boolean isSelling) {
        int itemId = CupidBot.getRs2ItemManager().getItemId(itemName);
        if (itemId == -1) {
            return null;
        }
    ...
    static int getOfferQuantity() {
        return CupidBot.getVarbitValue(VarbitID.GE_NEWOFFER_QUANTITY);
    }

    static int getOfferPrice() {
        return CupidBot.getVarbitValue(4398);
    }
    ...
    public static void setChatboxValue(int value) {
        ...
        CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            CupidBot.getClient().setVarcStrValue(VarClientStr.INPUT_TEXT, String.valueOf(value));
            return null;
        });
    ...
    public static boolean restoreOffer(Map<String, Object> offerDetails, GrandExchangeSlots targetSlot) {
        ...
        try {
            ...
            String itemName = CupidBot.getClient().getItemDefinition(itemId).getName();
    ...
    public static GrandExchangeOfferDetails hasBuyOffer(int itemId) {
        for (GrandExchangeSlots GrandExchangeSlot : GrandExchangeSlots.values()) {
            GrandExchangeOffer offer = CupidBot.getClient().getGrandExchangeOffers()[GrandExchangeSlot.ordinal()];
